...
module.exports = class JRecurlyPlan extends jraphical.Module
  ...
  @updateCache = (callback)->
    ...
    payment.getPlans (err, plans)->
      ...
      plans.forEach (plan)->
        stack.push (cb)->
          {code, title, desc, feeMonthly, feeInitial} = plan
          unless code.match /^([a-zA-Z0-9-]+_){3}[0-9]+$/
            cb()
          else
            JRecurlyPlan.one
            ...
            , (err, recurlyPlan)->
              ...
              unless recurlyPlan
                recurlyPlan = new JRecurlyPlan
                recurlyPlan.code = code

              recurlyPlan.title = title
              recurlyPlan.desc = desc
              recurlyPlan.feeMonthly = feeMonthly
              recurlyPlan.feeInitial = feeInitial

              [prefix, category, item, version] = code.split '_'
              version = +version
              recurlyPlan.product = {prefix, category, item, version}
