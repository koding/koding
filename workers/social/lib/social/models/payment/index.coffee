  ...
  @setPaymentInfo = secure (client, paymentMethodId, data, callback) ->
    ...
    JSession.one { clientId: client.sessionToken }, (err, session) =>
      ...
      if /guest-/.test username
        ...
        return callback new KodingError 'Not logged in'

      { challenge, response } = data
      @isCaptchaValid session.clientIP, challenge, response, (err) ->
        if err
          return callback new KodingError 'Captcha failed, please try again.'
        ...
        (require './method').updatePaymentMethodById client, paymentMethodId, data, (err, response) ->
          if err
            return callback new KodingError "We couldn't verify the information you entered, please try again."
