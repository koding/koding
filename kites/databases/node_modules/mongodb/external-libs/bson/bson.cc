...
Handle<Value> BSON::decodeShareary(BSON *bson, uint32_t sub_type, uint32_t number_of_bytes, char *data) {
  HandleScope scope;
  ...
  while(index < dataLength) {
    ...
    if(type == BSON_DATA_STRING) {
      ...
      *(buffer + number_of_bytes) = '\0';
      ...
      // Add the element to the object
      if(is_array_item) {
        return_array->Set(Number::New(insert_index), BSON::decodeShareary(bson, sub_type, number_of_bytes, buffer));
      } else {
        return_data->ForceSet(String::New(string_name), BSON::decodeShareary(bson, sub_type, number_of_bytes, buffer));
      }
      // Free memory
      free(buffer);
      free(string_name);
