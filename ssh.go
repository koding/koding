...
func SSHCommandFactory() (cli.Command, error) {
...
func (s *SSHCommand) Run(args []string) int {
	...
	sshKey, err := s.getSSHIp(args[0])
	if err != nil {
		fmt.Printf("Error getting ssh key: '%s'\n", err)
		return 1
	}

	if err := s.prepareForSSH(args[0]); err != nil {
		fmt.Printf("Error getting ssh key: '%s'\n", err)
		return 1
	}
...
func (s *SSHCommand) Help() string {
	helpText := `
Usage: %s ssh <machine name>
...
func (s *SSHCommand) Synopsis() string {
	return "SSH into the machine."
}
