...
docker run  --expose=80      --volume=$LOG:$LOG   --net=host -d --name=webserver        --entrypoint=node koding/codebase $PRJ/server/index.js               -c $CFG -p 80   --disable-newrelic
docker run  --expose=3526    --volume=$LOG:$LOG   --net=host -d --name=sourceMapServer  --entrypoint=node koding/codebase $PRJ/server/lib/source-server.js   -c $CFG -p 3526
docker run                   --volume=$LOG:$LOG   --net=host -d --name=authWorker       --entrypoint=node koding/codebase $PRJ/workers/auth/index.js         -c $CFG
docker run  --expose=3030    --volume=$LOG:$LOG   --net=host -d --name=social           --entrypoint=node koding/codebase $PRJ/workers/social/index.js       -c $CFG -p 3030 --disable-newrelic --kite-port=13020
docker run                   --volume=$LOG:$LOG   --net=host -d --name=guestCleaner     --entrypoint=node koding/codebase $PRJ/workers/guestcleaner/index.js -c $CFG
docker run                   --volume=$LOG:$LOG   --net=host -d --name=emailSender      --entrypoint=node koding/codebase $PRJ/workers/emailsender/index.js  -c $CFG
