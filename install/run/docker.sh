...
docker run  --expose=4000    --volume=$LOG:$LOG   --net=host -d --name=kontrol          --entrypoint=$PRJ/go/bin/kontrol       koding/run -c $CFG -r $RGN
docker run                   --volume=$LOG:$LOG   --net=host -d --name=kloud            --entrypoint=$PRJ/go/bin/kloud         koding/run -c $CFG -r $RGN -public-key $PBKEY -private-key $PVKEY -kontrol-url "http://$HST:4000/kite"
docker run                   --volume=$LOG:$LOG   --net=host -d --name=rerouting        --entrypoint=$PRJ/go/bin/rerouting     koding/run -c $CFG
docker run                   --volume=$LOG:$LOG   --net=host -d --name=cronJobs         --entrypoint=$PRJ/go/bin/cron          koding/run -c $CFG
docker run  --expose=8008    --volume=$LOG:$LOG   --net=host -d --name=broker           --entrypoint=$PRJ/go/bin/broker        koding/run -c $CFG
docker run  --expose=4001    --volume=$LOG:$LOG   --net=host -d --name=proxy            --entrypoint=$PRJ/go/bin/reverseproxy  koding/run -region $RGN -host $HST -env production 
...
docker run  --expose=3526    --volume=$LOG:$LOG   --net=host -d --name=sourceMapServer  --entrypoint=node koding/run $PRJ/server/lib/source-server.js   -c $CFG -p 3526
