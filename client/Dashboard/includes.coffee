module.exports = [

  # GROUP DASHBOARD
  "AppController.coffee",
  "AppView.coffee",

  # Group Dashboard Views:
  "views/linkablepaymentmethodview.coffee",
  "views/paymentsettingsview.coffee",

  # Product Pane
  # controller:
  "controllers/paymentcontroller.coffee",
  "controllers/productscontroller.coffee",
  "controllers/invitationrequestlistcontroller.coffee",
  # views:
  "views/administrationview.coffee",
  "views/generalsettingsview.coffee",
  "views/productsettingsview.coffee",
  "views/productsectionview.coffee",
  "views/producteditform.coffee",
  "views/packeditform.coffee",
  "views/embedcodeview.coffee",
  "views/productview.coffee",
  "views/productadmincontrols.coffee",
  "views/productlistitem.coffee",
  "views/childproductlistitem.coffee",
  "views/planadmincontrols.coffee",
  "views/planlistitem.coffee",
  "views/productsectionlistcontroller.coffee",
  "views/addproductlistitem.coffee",
  "views/planaddproductsmodal.coffee",
  "views/groupsmemberpermissionslistitemview.coffee",
  "views/groupsmemberpermissionsview.coffee",
  "views/groupsmemberroleseditview.coffee",
  "views/groupsmembershippolicydetailview.coffee",
  "views/groupsinvitationview.coffee",
  "views/permissionview.coffee",
  "views/permissionswitch.coffee",
  "views/blockeduserslistitemview.coffee",
  "views/blockedusersview.coffee",
  "views/groupsinvitationcodelistitemview.coffee"
  "views/groupsinvitationlistitemview.coffee"
  "views/groupsinvitationtabpaneview.coffee"
  "views/groupsinvitationtabview.coffee"
  "views/grouptabhandleview.coffee"
  "views/groupsuploadview.coffee"
  "views/groupsdangermodalview.coffee"
  "views/groupswebhookview.coffee"
  "views/groupseditablewebhookview.coffee"
  "views/groupsmembershippolicyeditor.coffee"
  "views/groupsformgeneratorview.coffee"

  # customviews
  "views/customviews/customviewsdashboardview.coffee",
  "views/customviews/customviewitem.coffee",
  "views/customviews/homepagecustomviewitem.coffee",
  "views/customviews/widgetcustomviewitem.coffee",
  "views/customviews/addnewcustomviewform.coffee",
  "views/customviews/customviewsmanager.coffee",

  # onboarding
  "views/onboarding/onboardingdashboardview.coffee",
  "views/onboarding/onboardingsectionform.coffee",
  "views/onboarding/onboardingitemview.coffee",
  "views/onboarding/onboardingaddnewform.coffee",

  "views/permissionsform.coffee",
  "views/grouplogosettings.coffee",

  "styl/app.group.dashboard.styl",
  "styl/onboarding.styl"
]
