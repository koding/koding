...
describe 'ChatInputChannelsQueryStore', ->
  ...
  describe '#setQuery', ->
    ...
    it 'sets current query to a given value', ->
      ...
      @reactor.dispatch actionTypes.SET_CHAT_INPUT_CHANNELS_QUERY, query : query1
      query = @reactor.evaluate ['chatInputChannelsQuery']

      expect(query).to.equal query1

      @reactor.dispatch actionTypes.SET_CHAT_INPUT_CHANNELS_QUERY, query: query2
      query = @reactor.evaluate ['chatInputChannelsQuery']
  ...
  describe '#unsetQuery', ->
    ...
    it 'clears current query', ->
      ...
      @reactor.dispatch actionTypes.SET_CHAT_INPUT_CHANNELS_QUERY, query : query
      query = @reactor.evaluate ['chatInputChannelsQuery']

      expect(query).to.equal query

      @reactor.dispatch actionTypes.UNSET_CHAT_INPUT_CHANNELS_QUERY
      query = @reactor.evaluate ['chatInputChannelsQuery']
