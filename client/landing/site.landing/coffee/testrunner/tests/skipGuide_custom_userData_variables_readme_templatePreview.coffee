...
module.exports = ->
  ...
  describe 'skipGuide_custom_userData_variables_readme_templatePreview', ->
    ...
    describe "Click on 'Credentials' text having a red (!) next to it?", ->
      ...
      it "Do you see 'rainforestqa99's AWS keys'?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see 'USE THIS & CONTINUE' button next to it?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see 'ADD A NEW CREDENTIAL' button on the bottom??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Move your mouse over 'USE THIS & CONTINUE' button?", ->
      ...
      it "Have 'PREVIEW' and 'DELETE' texts appeared??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on 'PREVIEW'?", ->
      ...
      it "Do you see 'rainforestqa99's AWS keys Preview' title on a pop-up displayed?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see 'access_key', 'region', 'secret_key' and 'identifier' sections??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on 'Edit Name' link?", ->
      ...
      it "Is '_Aws Stack' converted to a textbox??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Delete the text and write 'Team Stack' instead of it and hit enter?", ->
      ...
      it "Is it updated successfully as 'Team Stack' also on left sidebar under 'STACKS' section??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on 'Readme' tab?", ->
      ...
      it "Do you see '###### Stack Template Readme' text in the first line?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see 'PREVIEW' button on the bottom right??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Delete the text and copy and paste '# WELCOME TO KODING! You've said goodbye to your localhost and are ready to develop software in the cloud!' there and click on 'PREVIEW' button?", ->
      ...
      it "Do you see 'Readme Preview' title on a pop-up displayed?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see the text you pasted??", (done) -> 
        assert(false, 'Not Implemented')
        done()

    describe "Close that modal by clicking (x) from top right corner, click on 'Team Stack' tab and click on 'PREVIEW' button on the right bottom?", ->
      before (done) -> 
        # implement before hook 
        done()


      it "# Do you see 'Template Preview' title on a pop-up displayed??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on EXPAND?", ->
      ...
      it "Has that pop-up expanded?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see 'COLLAPSE' button on top right??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on 'CANCEL'?", ->
      ...
      it "Do you see 'MAKE TEAM DEFAULT', 'INITIALIZE' and 'SAVE' buttons on top right??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on 'INITIALIZE' button and wait for a few seconds for process to be completed?", ->
      ...
      it "Do you see 'Team Stack' title?", (done) -> 
        assert(false, 'Not Implemented')
        done()
      ...
      it "Do you see 'Read Me' and 'WELCOME TO KODING' texts?", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on the 'NEXT' button?", ->
      ...
      it "Do you see 'Requirements:' title, 'Requirement Selection' text and a text box on the right-side?", (done) -> 
        assert(false, 'Not Implemented')
        done()

      it "Do you see '+ Create New' link below??", (done) -> 
        assert(false, 'Not Implemented')
        done()
    ...
    describe "Click on the tab where it says 'Terminal', type 'ls /' and hit enter?", ->
      ...
      it "Do you see the green colored 'helloworld.txt', 'qa_test' and '{{ random.first_name }}' files in the list??", (done) -> 
        assert(false, 'Not Implemented')
        done()
