...
StackProgressModal = require './stackprogressmodal'
remote = require('app/remote').getInstance()
nick = require 'app/util/nick'
subdomainPattern = require('app/util/regexps').subdomainPattern


module.exports = class CloneStackModal extends KDModalView
  ...
  createDomainCreateForm: (domain) ->
    ...
    form.once "viewAppended", =>
      ...
      splitted.first = ""  if splitted.first is nick()
  ...
  validateDomains: ->
    ...
    remote.api.JProposedDomain.fetchDomains (err, domains) =>
      ...
      for domainName, index in @newDomains
        ...
        isValid           = subdomainPattern.test domainNameInput.getValue()
  ...
  addDomainsToQueue: ->
    ...
    @domainCreateForms.forEach (form, index) =>
      ...
      @queue.push =>
        remote.api.JProposedDomain.createDomain { domain: newName, stack: @stack.getId() }, (err, res) =>
          if err
  ...
  createStack: (callback = noop) ->
    # remote.api.JComputeStack.createStack @getOptions().meta, (err, @stack) =>
