class LoginInputView extends JView
  ...
  constructor:(options = {}, data)->
    ...
    @placeholderHelper = new KDCustomHTMLView
      cssClass : 'placeholder-helper'
      partial  : inputOptions.hint or inputOptions.placeholder or inputOptions.name

    @input.on 'focus', => @placeholderHelper.setClass 'in'
    @input.on 'blur', => @placeholderHelper.unsetClass 'in'
  ...
  pistachio:-> "{{> @input}}{{> @icon}}{{> @placeholderHelper}}"
...
class LoginInputViewWithLoader extends LoginInputView
  ...
  pistachio:-> "{{> @input}}{{> @icon}}{{> @loader}}{{> @placeholderHelper}}"
