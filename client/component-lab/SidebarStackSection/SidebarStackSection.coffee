...
module.exports = class SidebarStackSection extends React.Component
  ...
  onMenuDraftItemClick: (item, event) ->
    ...
    MENU.destroy()
    { stack, template, credential } = @props
    { title } = item.getData()
    switch title
      ...
      when 'Make Team Default' then @props.makeTeamDefault template, credential[0]  if credential
  ...
  onMenuItemClick: (item, event) ->
    ...
    { stack, template, machines, credential } = @props
    { title } = item.getData()
