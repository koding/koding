...
if ! /sbin/pvcreate ${PV} 2>>${LOG} 1>/dev/null ;then
	...
	if ! /sbin/vgcreate ${VG} ${PV} 2>>${LOG} 1>/dev/null;then
		...
		if ! /sbin/lvcreate -n ${LV_DBS} -l50%VG ${VG} 2>>${LOG} 1>/dev/null ;then
			echo "Can't create logical volume ${LV_DBS}" >>${LOG}
			exit 1
		else
			echo "LV ${LV_DBS} has been created" >>${LOG}
			if ! /sbin/mkfs.ext4 /dev/${VG}/${LV_DBS} 2>>${LOG} ;then
				echo "Can't create FS /dev/${VG}/${LV_DBS}" >>${LOG}
				exit 1
			else
				echo "FS /dev/${VG}/${LV_DBS} has been created" >>${LOG}
			fi
		fi
		if ! /sbin/lvcreate -n ${LV_BINLOGS} -l30%FREE ${VG} 2>>${LOG} 1>/dev/null ;then
			echo "Can't create logical volume ${LV_BINLOGS}" >>${LOG}
			exit 1
		else
			echo "LV ${LV_BINLOGS} has been created" >>${LOG}
			if ! /sbin/mkfs.ext4 /dev/${VG}/${LV_BINLOGS} 2>>${LOG} ;then
				echo "Can't create FS /dev/${VG}/${LV_BINLOGS}" >>${LOG}
				exit 1
			else
				echo "FS /dev/${VG}/${LV_BINLOGS} has been created" >>${LOG}
			fi
		fi
	fi
fi
