...
class KiteRequestHandler
  ...
  def join_all()
    ...
    channels = {}
    begin
      dir = Dir.new(@config[:joined_channels_path])
    rescue Errno::ENOENT
      FileUtils.mkdir_p(@config[:joined_channels_path])
      retry
    end
    $log.debug dir.inspect
    dir.each do |channel|
      if channel !~ /^(\.\.?)$/ && channels[channel].nil?
  ...
  def remove_from_joined_channels_path(channel)
    unless channel.nil?
      ...
      begin
        ...
        $log.warn "[WARN] could not remove tmp file for #{channel}"
      end
    end
  end
  
  def status(event)
    $log.debug 'BEGINNING STATUS DUMP'
    $log.debug "REQUEST-HANDLER LEVEL RECORD: \n\t"+@channels.keys.join("\n\t")
    $log.debug "SOCKET LEVEL RECORD: \n\t"+@socket.channels.channels.keys.join("\n\t")
  ...
  def part(channel, silent=false)
    ...
    $log.debug channel['channel'].inspect
