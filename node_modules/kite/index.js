...
module.exports = Kite = (function() {
  ...
  Kite.prototype.startRequestHandler = function(config) {
    var cmd, hostname, kiteUri, logfile, port, protocol, requestHandler, requestHandlerPath;
    ...
    cmd = ["cd " + requestHandlerPath + " &&", 'ruby index.rb', '-k', config.pusher.key, '-s', config.pusher.secret, '-l', logfile, '-u', kiteUri, '-n', this.kiteName].join(' ');
    ...
    return requestHandler.on('exit', function(code) {
      return log('request handler child process exited with code ' + code);
    });
  };
  Kite.prototype.handleRequest = function(data, res) {
    var channels, requesterId, requesters, scrubbed;
    log(this.kiteName, 'HANDLING REQUEST', data);
    ...
    return this.unscrubMessage(requesterId, scrubbed, __bind(function(unscrubbed) {
      var callback, callbackId, kallback, secretChannelId, subscriberCount, toDo, withArgs, _base, _base2, _base3, _ref3;
      ...
      if (requesters[requesterId] == null) {
        ...
        this.pusher.trigger("private-krh-" + this.kiteName, 'join', "private-" + this.kiteName + "-" + requesterId);
      ...
      switch (toDo) {
        ...
        default:
          if (this.api.propertyIsEnumerable(toDo) && 'function' === typeof this.api[toDo]) {
            if ((_ref3 = channels[secretChannelId]) == null) {
  ...
  Kite.prototype.handleRequests = function() {
    this.app.post('/', __bind(function(req, res) {
      return this.handleRequest(req.body, res);
    }, this));
    return this.app.get('/', __bind(function(req, res) {
      return this.handleRequest(req.query, res);
    }, this));
  };
