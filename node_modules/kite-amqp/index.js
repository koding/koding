...
module.exports = Kite = (function() {
  ...
  Kite.prototype.handleRequest = function(message, kiteConfig, _arg) {
    var channels, deliveryInfo, exchange, exchangeName, headers, privateExchangeName, replyTo, requesterId, requesters, scrubbed,
      _this = this;
    headers = _arg.headers, deliveryInfo = _arg.deliveryInfo, exchange = _arg.exchange;
    ...
    try {
      ...
      requesterId = privateExchangeName[privateExchangeName.length - 2];
    } catch (err) {
    ...
    return this.unscrubMessage(exchange, replyTo, requesterId, scrubbed, function(unscrubbed) {
      var callback, callbackId, data, method, withArgs, _base, _base1, _base2, _ref1;
      data = unscrubbed[0], callback = unscrubbed[1];
      console.log(unscrubbed);
      method = data.method, withArgs = data.withArgs, callbackId = data.callbackId;
      withArgs || (withArgs = {});
  ...
  Kite.prototype.join = function(channel, config) {
    var _this = this;
    return this.connection.exchange('' + channel, this.getExchangeOptions(), function(exchange) {
      return _this.connection.queue('' + channel + '-requests', function(queue) {
    ...
    return this.connection.on('ready', function() {
      return _this.connection.exchange(_this.getExchangeName(), _this.getExchangeOptions(), function(exchange) {
        return _this.connection.queue(_this.getQueueName(), function(queue) {
          ...
          return queue.on('queueBindOk', function() {
            return queue.subscribe(function(message, headers, deliveryInfo) {
              return _this.join(message.data, config);
            });
          });
        });
      });
