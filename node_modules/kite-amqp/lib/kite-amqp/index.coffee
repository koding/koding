...
module.exports = class Kite
  ...
  handleRequest:(message, kiteConfig, {headers, deliveryInfo, exchange})->
    ...
    @unscrubMessage exchange, replyTo, requesterId, scrubbed, (unscrubbed)=>
      ...
      {method, withArgs, callbackId} = data
      withArgs or= {}
      ...
      switch method
        ...
        when '_ping' then @handleResponse replyTo, callback.id, 1
          ...
          else @handleError exchange, replyTo, callback.id, {
  ...
  join:(channel, config)->
    @connection.exchange ''+channel, @getExchangeOptions(), (exchange)=>
      @connection.queue ''+channel+'-requests', (queue)=>
        queue.bind exchange, 'client-message.#'
        queue.on 'queueBindOk', =>
          queue.subscribe (message, headers, deliveryInfo)=>
            @handleRequest message, config, {headers, deliveryInfo, exchange}
