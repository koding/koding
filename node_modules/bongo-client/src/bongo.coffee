...
class Bongo extends EventEmitter
  ...
  createRemoteApiShims:(api)->
    shimmedApi = {}
    for name, {statik, instance} of api
      shimmedApi[name] = @createConstructor name, statik, instance
    shimmedApi
  ...
  connect:(callback)->
    ...
    @channel.once 'broker:subscription_succeeded', =>
      ...
      setTimeout =>
        @channel.emit 'client-join', 'whats really happening'
      , 1000
  ...
  getTimeout:(message, timeout=5000)->
    setTimeout @messageFailed.bind(@, message), timeout
  ...
  send:(method, args)->
    ...
    unless @channel
      ...
      scrubber.scrub args, =>
        ...
        # message.timeout = @getTimeout(message)
