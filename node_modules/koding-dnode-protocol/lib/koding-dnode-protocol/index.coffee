  ...
  handle:(msg)->
    ...
    args = scrubber.unscrub msg, (callbackId)=>
      unless @remoteStore.has(callbackId)
        @remoteStore.add callbackId, => @request callbackId, [].slice.call arguments
...
exports.Scrubber = class DnodeScrubber extends Scrubber
  constructor:(store=new DnodeStore)->
    @paths = {}
    @links = []
    dnodeMutators = [    
      # scrub function refs
      ...
      (cursor)->
        if cursor.circular
          @links.push
            ...
            to    : cursor.path
          cursor.update '[Circular]'
    ]
    userStack = DnodeScrubber.stack or []
