module.exports = class Broker
  ...
  bindExchange: (destination, source, routingKey) ->
    @ready =>
      ...
      queue = [
        createDestination = =>
          @connection.exchange destination, exchangeOptions, (exchange) =>
            exchanges.destination = exchange
            queue.fin()
        createSource = =>
          @connection.exchange source, exchangeOptions, (exchange) =>
  ...
  bindQueue: (name, exchange, routing, done) ->
    @ready =>
      unless exchange in (@connection.exchanges or [])
        @connection.exchange exchange, {durable: true}, =>
          createQueue @connection, name, (queue) ->
