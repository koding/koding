module.exports = class Broker
  ...
  createExchange: (exchangeName, options, done) ->
    @ready =>
      ...
      @connection.exchange exchangeName, options, done

  createQueue: (queueName, options, done) ->
    @ready =>
      if typeof options is 'function'
        done = options
        options = {durable:true, exclusive:true} 
      @connection.queue queueName, options, done
  ...
  bindQueue: (name, exchange, routing, options, done) ->
    @ready =>
      options ?= {}
      {exchangeDurable, exchangeAutoDelete, queueDurable, queueExclusive} = options
      queueOptions =
        durable: if ('undefined' is typeof queueDurable) then true else queueDurable
        exclusive: if ('undefined' is typeof queueExclusive) then true else queueExclusive

      unless exchange of (@connection.exchanges or {})
        exchangeOptions =
          durable: if ('undefined' is typeof exchangeDurable) then true else exchangeDurable
          autoDelete: if ('undefined' is typeof exchangeAutoDelete) then true else exchangeAutoDelete
        
        @createExchange exchange, exchangeOptions, =>
          @createQueue name, queueOptions, (queue) ->
            queue.bind exchange, routing
            queue.on 'queueBindOk', done if done?
      else
        @createQueue name, queueOptions, (queue) ->
          queue.bind exchange, routing
          queue.on 'queueBindOk', done if done?
  ...
  on: (channel, event, queueName, listener) ->
    ...
    @ready =>
      unless channel of (@connection.exchanges or {})
        @createExchange channel, =>
          @subscribe queueName, listener
          @bindQueue queueName, channel, event
      else
        @subscribe queueName, listener
        @bindQueue queueName, channel, event
