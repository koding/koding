...
module.exports = class Module extends Model
  # 
  # Object.defineProperty @, 'clearUpstreamSnapshots', value:do ->
  #   
  #   clearUpstreamSnapshots_ =(ids, register)->
  #     if id = ids.shift()
  ...
  #         as          : 1
  #       }, {}, (err, refs)->
  #         refs.toArray (err, arr)->
  #           groups = groupBy(arr, 'sourceName')
  #           queue = []
  #           Object.keys(groups).forEach (name)->
  #             group = groups[name]
  #             queue.push ->
  #               unseenNodeIds = (item.sourceId for item in group when register.sign item.sourceId)
  #               constructor = Base.constructors[name]
  #               constructor.update {
  #                 _id: $in: unseenNodeIds
  #               }, {
  #                 $unset: snapshot: 1
  #               }, ->
  #                 ids.push unseenNodeIds...
  #                 queue.next()
  #           queue.push ->
  #             clearUpstreamSnapshots_(ids, register) # recurse
  #           daisy queue
  #   # @implementation: Module.clearUpstreamSnapshots(node)
  #   (node)->
  #     register = new Register
  #     nodeId = node.getId()
  #     register.sign(nodeId)
  #     clearUpstreamSnapshots_([nodeId], register)
  # 
  ...
  getFetcher_ =(constructor, name, def, onlyOne=no)->
    ...
    ->
      ...
      otherConstructor = def[outgoingType]
      idField = "#{orientation}Id"
      fields = {}
      fields[idField] = yes
      fields['data.flags'] = yes
      fields["#{orientation}Name"] = yes
            ...
            if err
              ...
              collectOthers = race (i, type, group, next)->
                ...
                if targetSelector = targetOptions.selector or targetOptions.query
                  ...
                  targetSelector = {}
                unless Base?.constructors?[type]
                  console.log "[ERROR] bongo err on:",{selector,fields,options}
              ...
              # ids = (rel[idField] for rel in docs)
              # flags = {}
              # ids.forEach (id, index)->
              #   flag = docs[index].data?.flags
              #   flags[id] = flag if flag
              # targetOptions or= {}
              # if targetSelector = targetOptions.selector or targetOptions.query
              #   delete targetOptions.selector
              ...
              # else
              #   targetSelector = {}
              # otherConstructor.some extend(targetSelector,
              #   _id: $in: ids
              # ), targetOptions, (err, others)->
              #   if others? 
              #     others = for other in others
  ...
  getAdder_ =(constructor, name, def)->
    (target, options, callback)->
      ...
      unless target.constructor is targetType or
        ...
        if Array.isArray(targetType) and target.constructor in targetType
