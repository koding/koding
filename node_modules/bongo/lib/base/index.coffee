...
module.exports = class Base extends EventEmitter2
  ...
  ###
  {@on, @off, @addListener, @removeListener, @removeAllListeners, @onAny, @offAny, @many, @once
   @listeners, @listenersAny, @setMaxListeners, @emit, @getEmitter} = require './staticemitterdelegate'
  ...
  @param: staticMembers - members to be mixed into the constructor.
  ...
  ###
  @extend = (staticMembers, proto)->
    props = [staticMembers, proto]
    for prop, index in props
      if 'function' is typeof prop
        props[index] = new prop
    [proto, staticMembers] = props unless proto
    parent = @
  ...
  @param: methods - an object containing the following
    properties:
    @property: staticMembers - an array of method names to share 
  ...
  @share =->
    ...
    Base.sharedConstructors[name] = constructor
    @
  ...
  emit:(event, data)->
    ...
    if id?
      if Array.isArray event
        event = event.join('.')
      @mq?.trigger "object-#{id}", event, data
