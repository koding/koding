...
module.exports = class Model extends Base
  ...
  @getCollection =->
    ...
    unless client
      throw new Error \
        """
        You must set the client for this constructor, or failing that, you must set a generic client for Model.
        """
  ...
  @getMemosOf = (schema)->
    ...
    memos = 
      paths               : {}
      filters             : {}
      # broadcastChanges    : {} # TODO: implement leaf-level broadcastChanges
      types               : {}
      subcollections      : {}
      subcollectionEmbeds : {}
      validators          : {}
  ...
  ###
  {@one, @all, @some, @someData, @remove, @removeById,
   @count, @teasers, @hose, @mapReduce, @assure, @update} = require './find'
  ###
  Feeding
  ...
  save_0_ =(callback, err)->
    ...
    if err?
      ...
      unless model.data._id?
        ...
        collection.insert data, (safe:yes), save_1_.bind model, callback, yes
      else
        ...
        collection.update \
          ...
          (safe:yes), save_1_.bind model, callback, no
    else
  ...
  @param: model - to bind the function to
  ### 
  save_1_ =(callback, isNew, err, docs)->
    model = @
    {constructor} = model
    unless err
      if id = docs[0]?._id
        model.data._id = id
      if constructor.feedable
...
#            @update @node, yes
  ...
  prune:do ->
    ...
    (isHard=no)->
      ...
      {constructor} = model
      # data = model.get()
      prunedData = new Traverse(model.data)[if isHard then 'forEach' else 'map'] (node)->
        pathName = @path.join '.'
        ...
        if pathName is '_id'
          ...
          prunedCollection = (embed.prune() for embed in node)
  ...
  update:(atomically, callback=->)->
    ...
    unless atomically
      ...
      for own path, value of atomically
        # if /^$/.test path then use$Set = yes
        type = Model.getType model, path.split '.'
        if type and type isnt Object
          value = Model.castValue value, type
          atomically[path] = value
      new MongoOp(atomically).applyTo(model.data) # first update the one in memory.
      collection.update update, atomically, (err)-> # then update the one in the database.
        if err
          callback err
        else
  ...
  remove:(callback)-> @constructor.remove _id: @getId(), callback
