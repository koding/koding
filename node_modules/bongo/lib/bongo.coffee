...
module.exports = class Bongo extends EventEmitter
  ...
  connect:(callback)->
    @mq.on 'bongo', 'join', (secretName)=>
      setTimeout =>
        @mq.emit secretName, 'server-message', 'ready'
      , 500
  ...
  handleRequest:(secretName, message)->
    ...
    if method is 'fetchApi'
      ...
      @invokeMethod method, unscrubbed
      ...
      switch method.type
        when 'static'
          if konstructor.hasSharedMethod method.method
            fn = konstructor[method.method]
            fn.apply konstructor, unscrubbed
          else @handleError 'No such method'
        when 'instance'
          if konstructor::hasSharedMethod method.method
            if method.id
  ...
  describeApi:(callback)->
    api = {}
    for own name, konstructor of Base.constructors
      api[name] = konstructor.getSharedMethods()
      api[name].options = {
