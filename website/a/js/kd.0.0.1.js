...
!function(){var Pistachio,__slice=Array.prototype.slice,__hasProp=Object.prototype.hasOwnProperty
...
this.html=this.init()}var cleanSubviewNames,pistachios,symbolKeys
...
symbol={tagName:tagName,id:id,isCustom:isCustom,js:js,code:code,render:render,dataPaths:dataPaths,subViewNames:subViewNames}
...
Pistachio.prototype.addSymbolInternal=function(symbol){var dataPath,dataPaths,subViewName,subViewNames,_base,_base2,_i,_j,_len,_len2
dataPaths=symbol.dataPaths,subViewNames=symbol.subViewNames
this.symbols[symbol.id]=symbol
for(_i=0,_len=dataPaths.length;_len>_i;_i++){dataPath=dataPaths[_i]
null==(_base=this.symbolsByDataPath)[dataPath]&&(_base[dataPath]=[])
this.symbolsByDataPath[dataPath].push(symbol)}for(_j=0,_len2=subViewNames.length;_len2>_j;_j++){subViewName=subViewNames[_j]
null==(_base2=this.symbolsBySubViewName)[subViewName]&&(_base2[subViewName]=[])
this.symbolsBySubViewName[subViewName].push(symbol)}return symbol}
...
Pistachio.prototype.appendChild=function(childView){return this.addAdhocSymbol(childView)}
Pistachio.prototype.prependChild=function(childView){return this.addAdhocSymbol(childView)}
...
symbolKeys={subview:"symbolsBySubViewName",path:"symbolsByDataPath"}
Pistachio.prototype.refreshChildren=function(childType,items,forEach){var el,id,item,out,symbol,symbols,unique,_i,_j,_len,_len2,_results
null==forEach&&(forEach=function(){})
unique={}
for(_i=0,_len=items.length;_len>_i;_i++){item=items[_i]
symbols=this[symbolKeys[childType]][item]
if(null!=symbols)for(_j=0,_len2=symbols.length;_len2>_j;_j++){symbol=symbols[_j]
unique[symbol.id]=symbol}}_results=[]
for(id in unique)if(__hasProp.call(unique,id)){symbol=unique[id]
el=document.getElementById(id)
if(null!=el){out=null!=symbol?symbol.render():void 0
out?_results.push(forEach.call(el,out)):_results.push(void 0)}}return _results}
...
target[key]=val}}return target},dict:Object.create.bind(null,null,Object.create(null)),getNearestElementByTagName:function(el,tagName){for(;null!=el&&!this.elementHasTag(el,tagName);)el=el.parentNode
return el},elementShow:function(el){return null!=el?el.classList.remove("hidden"):void 0},elementHide:function(el){return null!=el?el.classList.add("hidden"):void 0},elementHasTag:function(el,tagName){var _ref
return Boolean((null!=(_ref=el.tagName)?_ref.toLowerCase():void 0)===tagName.toLowerCase())},elementIsVisible:function(el){var height,i,inViewport,l,onTop,r,rects
if(el.offsetWidth<=0||el.offsetHeight<=0)return!1
height=document.documentElement.clientHeight
rects=el.getClientRects()
onTop=function(r){var x,y
x=(r.left+r.right)/2
y=(r.top+r.bottom)/2
...
if(inViewport&&onTop(r))return!0
i++}return!1},formatPlural:function(count,noun,showCount){null==showCount&&(showCount=!0)
...
s=t
return s},applyMarkdown:function(text){return text?marked(Encoder.htmlDecode(text),{gfm:!0,pedantic:!1,sanitize:!0,highlight:function(text,lang){return null!=hljs.LANGUAGES[lang]?hljs.highlight(lang,text).value:text}}):null},createExternalLink:function(href){var tag
...
option.key=key
_results.push(option)}return _results},arrayToObject:function(list,key){var dict,obj,_i,_len
dict={}
for(_i=0,_len=list.length;_len>_i;_i++){obj=list[_i]
null!=obj[key]&&(dict[obj[key]]=obj)}return dict},partition:function(list,fn){var item,result,_i,_len
result=[[],[]]
...
var KD,e,error,log,noop,prettyPrint,warn,_base,_ref,_ref1,_ref2,_ref3,_ref4,_ref5,__slice=[].slice,__hasProp={}.hasOwnProperty;(_base=Function.prototype).bind||(_base.bind=function(context){var args,_this=this
...
return{create:create,"new":create,debugStates:{},instances:{},introInstances:{},singletons:{},subscriptions:[],classes:{},utils:__utils,lastFuncCall:null,instancesToBeTested:{},registerInstance:function(anInstance){var introId
...
return null},getAllKDInstances:function(){return KD.instances},getKDViewInstanceFromDomElement:function(el){return this.instances[el.getAttribute("data-id")]},exportKDFramework:function(){var item,_ref6
_ref6=KD.classes
...
return instance.on("KDObjectWillBeDestroyed",function(){return delete _this.instancesToBeTested[key]})},getInstanceForTesting:function(key){return this.instancesToBeTested[key]}}}())
prettyPrint=noop
...
KDObject.prototype.setData=function(data){this.data=data
return null!=data?"function"==typeof data.emit?data.emit("update"):void 0:void 0}
...
return-1}
KDView=function(_super){function KDView(options,data){var o
null==options&&(options={})
o=options
...
this.devHacks()}var defineProperty,deprecated,eventNames,eventToMethodMap,overrideAndMergeObjects
...
KDView.prototype.defaultInit=function(options){var _this=this
...
options.draggable&&this.setClass("kddraggable")
...
return this.on("viewAppended",function(){var child,fireViewAppended,key,subViews,_i,_len,_results,_results1
_this.setViewReady()
_this.viewAppended()
...
if(Array.isArray(subViews)){_results=[]
for(_i=0,_len=subViews.length;_len>_i;_i++){child=subViews[_i]
_results.push(fireViewAppended(child))}return _results}if(null!=subViews&&"object"==typeof subViews){_results1=[]
for(key in subViews)if(__hasProp.call(subViews,key)){child=subViews[key]
_results1.push(fireViewAppended(child))}return _results1}})}
...
KDView.prototype.devHacks=function(){var _this=this
return this.on("click",function(event){if(event){if(event.metaKey&&event.altKey&&event.ctrlKey){log(_this.getData())
...
if(this.stopScrolling)return!1
...
KDRouter.prototype.pushRoute=function(path){return"undefined"!=typeof _gaq&&null!==_gaq?_gaq.push(["_trackPageview",path]):void 0}
...
qs.length&&(path+="?"+qs)
if(suppressListeners||!shouldPushState||replaceState||path!==this.currentPath){this.pushRoute(path)
...
KDWindowController.prototype.bindEvents=function(){var layers,_this=this
...
document.addEventListener("scroll",function(){var body,timer
timer=null
body=document.body
return function(event){_this.emit("ScrollHappened",event)
clearTimeout(timer)
body.classList.contains("onscroll")||body.classList.add("onscroll")
return timer=KD.utils.wait(200,function(){return body.classList.remove("onscroll")})}}(),!1)
...
addListener("click",function(e){var href,isHttp,nearestLink,_ref
nearestLink=KD.utils.getNearestElementByTagName(e.target,"a")
if(0===(null!=nearestLink?null!=(_ref=nearestLink.target)?_ref.length:void 0:void 0)){href=nearestLink.getAttribute("href")
isHttp=0===(null!=href?href.indexOf("http"):void 0)
if(isHttp)return nearestLink.target="_blank"
e.preventDefault()
if(href&&!/^#/.test(href))return KD.getSingleton("router").handleRoute(href)}},!1)
...
KDListViewController.prototype.showNoItemWidget=function(){var _ref
return 0===this.itemsOrdered.length?null!=(_ref=this.noItemView)?_ref.show():void 0:void 0}
KDListViewController.prototype.hideNoItemWidget=function(){var _ref
return null!=(_ref=this.noItemView)?_ref.hide():void 0}
...
KDListViewController.prototype.showLazyLoader=function(emitWhenReached){var itemClass,lazyLoaderOptions,spinnerOptions,wrapper,_this=this
null==emitWhenReached&&(emitWhenReached=!0)
this.noItemView&&this.getOptions().noItemFoundWidget&&this.hideNoItemWidget()
if(!this.lazyLoader){lazyLoaderOptions=this.getOptions().lazyLoaderOptions
lazyLoaderOptions||(lazyLoaderOptions={})
lazyLoaderOptions.itemClass||(lazyLoaderOptions.itemClass=KDCustomHTMLView)
null==lazyLoaderOptions.partial&&(lazyLoaderOptions.partial="Loading...")
lazyLoaderOptions.cssClass=KD.utils.curry("lazy-loader",lazyLoaderOptions.cssClass)
lazyLoaderOptions.spinnerOptions||(lazyLoaderOptions.spinnerOptions={size:{width:32}})
...
emitWhenReached&&this.emit("LazyLoadThresholdReached")
return KD.utils.defer(function(){var _ref
return null!=(_ref=_this.scrollView)?_ref.stopScrolling=!0:void 0})}}
KDListViewController.prototype.hideLazyLoader=function(){var _this=this
KD.utils.wait(300,function(){var _ref
return null!=(_ref=_this.scrollView)?_ref.stopScrolling=!1:void 0})
this.noItemView&&this.getOptions().noItemFoundWidget&&this.showNoItemWidget()
...
KDListItemView.prototype.dim=function(){return this.setClass("dimmed")}
KDListItemView.prototype.undim=function(){return this.unsetClass("dimmed")}
KDListItemView.prototype.highlight=function(){this.undim()
return this.setClass("selected")}
KDListItemView.prototype.removeHighlight=function(){this.undim()
...
options.enableMoveTabHandle&&this.setTabHandleMoveNav()
...
KDTabView.prototype.addPane=function(paneInstance,shouldShow){var maxHandleWidth,minHandleWidth,name,newTabHandle,paneOptions,tabHandleClass,_ref,_this=this
...
_ref=this.getOptions(),minHandleWidth=_ref.minHandleWidth,maxHandleWidth=_ref.maxHandleWidth
newTabHandle.getDomElement().css({maxWidth:maxHandleWidth,minWidth:minHandleWidth})
newTabHandle.on("HandleIndexHasChanged",this.bound("resortTabHandles"))
return paneInstance}name=(null!=(null!=paneInstance?paneInstance.constructor:void 0)).name
...
KDTabView.prototype.appendHandle=function(tabHandle){var enableMoveTabHandle,maxHandleWidth,_ref
...
return enableMoveTabHandle?this._tabsWidth=this.handles.length*maxHandleWidth:void 0}
KDTabView.prototype.addHandle=function(handle){var name
...
return handle}name=(null!=(null!=handle?handle.constructor:void 0)).name
return warn("You can't add "+(null!=name?name:void 0)+" as a pane, use KDTabHandleView instead")}
KDTabView.prototype.removeHandle=function(){}
KDTabView.prototype.showPane=function(pane){var activePane,handle,index
if(pane){activePane=this.getActivePane()
activePane&&(this.lastOpenPaneIndex=this.getPaneIndex(activePane))
this.hideAllPanes()
pane.show()
index=this.getPaneIndex(pane)
handle=this.getHandleByIndex(index)
...
_ref=this.panes
for(_i=0,_len=_ref.length;_len>_i;_i++){pane=_ref[_i]
pane&&pane.hide()}_ref1=this.handles
_results=[]
for(_j=0,_len1=_ref1.length;_len1>_j;_j++){handle=_ref1[_j]
handle&&_results.push(handle.makeInactive())}return _results}
...
this.tabHandleContainer.tabs=new KDView({cssClass:"kdtabhandle-tabs"})
...
KDTabView.prototype.setTabHandleMoveNav=function(){return this.tabHandleContainer.addSubView(new KDTabHandleMoveNav({delegate:this}))}
...
KDTabView.prototype.getPaneIndex=function(aPane){if(!aPane)throw new Error("no pane provided!")
...
KDTabView.prototype.resizeTabHandles=KD.utils.throttle(function(){var containerMargin,containerSize,handle,options,outerWidth,possiblePercent,visibleHandles,visibleTotalSize,_i,_j,_len,_len1,_ref,_results
if(this.getOptions().resizeTabHandles&&!this._tabHandleContainerHidden&&!this.blockTabHandleResize){visibleHandles=[]
visibleTotalSize=0
options=this.getOptions()
outerWidth=this.tabHandleContainer.tabs.$().outerWidth(!1)
...
visibleTotalSize+=handle.$().outerWidth(!1)}}possiblePercent=(containerMargin/visibleHandles.length).toFixed(2)
_results=[]
...
KDTabPaneView.prototype.show=function(){this.setClass("active")
this.unsetClass("kdhiddentab")
this.active=!0
return this.emit("KDTabPaneActive")}
KDTabPaneView.prototype.hide=function(){this.setClass("kdhiddentab")
...
var KDTabHandleMoveNav,__hasProp={}.hasOwnProperty,__extends=function(child,parent){function ctor(){this.constructor=child}for(var key in parent)__hasProp.call(parent,key)&&(child[key]=parent[key])
ctor.prototype=parent.prototype
child.prototype=new ctor
child.__super__=parent.prototype
return child}
KDTabHandleMoveNav=function(_super){function KDTabHandleMoveNav(options,data){null==options&&(options={})
...
this._current=0}__extends(KDTabHandleMoveNav,_super)
KDTabHandleMoveNav.prototype.viewAppended=function(){var _this=this
this.addSubView(this.left=new KDCustomHTMLView({cssClass:"leftButton",click:function(){return _this.move("right")}}))
this.addSubView(this.right=new KDCustomHTMLView({cssClass:"rightButton",click:function(){return _this.move("left")}}))
return this.listenWindowResize()}
KDTabHandleMoveNav.prototype._windowDidResize=function(){var delegate
delegate=this.getDelegate()
if(delegate.getWidth()>delegate._tabsWidth+50){this.move("initial")
return this.hide()}return this.show()}
KDTabHandleMoveNav.prototype.move=function(direction){var delegate,handleWidth,tabHandleContainer
tabHandleContainer=(delegate=this.getDelegate()).tabHandleContainer
handleWidth=delegate.getOption("maxHandleWidth")
switch(direction){case"left":if(handleWidth*delegate.handles.length+100<tabHandleContainer.getWidth()-this._current)return
this._current-=handleWidth
break
case"right":if(0===this._current)return
this._current+=handleWidth
break
case"initial":this._current=0}return tabHandleContainer.tabs.setCss("marginLeft",""+this._current+"px")}
return KDTabHandleMoveNav}(KDCustomHTMLView)
...
options.menuMaxWidth||(options.menuMaxWidth=272)
options.menuMinWidth||(options.menuMinWidth=172)
...
JContextMenu.prototype.positionContextMenu=function(){var event,expectedLeft,expectedTop,left,mainHeight,mainView,mainWidth,menuHeight,menuMaxWidth,menuMinWidth,menuWidth,options,style,top
options=this.getOptions()
event=options.event||{}
...
menuWidth=options.menuWidth,menuMaxWidth=options.menuMaxWidth,menuMinWidth=options.menuMinWidth
style={width:""+menuWidth+"px",top:top,left:left}
if(menuMaxWidth){style.width="auto"
style.maxWidth=""+menuMaxWidth+"px"}menuMinWidth&&(style.minWidth=""+menuMinWidth+"px")
...
return _this._clone.css({height:"auto",zIndex:1e5,width:$input.width(),borderTop:$input.css("border-top"),borderRight:$input.css("border-right"),borderBottom:$input.css("border-bottom"),borderLeft:$input.css("border-left"),paddingTop:$input.css("padding-top"),paddingRight:$input.css("padding-right"),paddingBottom:$input.css("padding-bottom"),paddingLeft:$input.css("padding-left"),wordBreak:$input.css("wordBreak"),fontSize:$input.css("fontSize"),fontWeight:$input.css("fontWeight"),lineHeight:$input.css("lineHeight"),whiteSpace:"pre-line"})})})
...
options.placeholder||(options.placeholder="")
null==options.tabNavigation&&(options.tabNavigation=!1)
...
break
case"html":value=element.innerHTML}return value===placeholder?"":value.trim()}
...
KDContentEditableView.prototype.click=function(){return this.editingMode&&!this.focused?this.focus():void 0}
KDContentEditableView.prototype.input=function(event){return this.emit("ValueChanged",event)}
KDContentEditableView.prototype.keyDown=function(event){var maxLength,multiline,tabNavigation,validate,value,_ref,_ref1,_ref2
if(event.target===this.getElement()){_ref=this.getOptions(),tabNavigation=_ref.tabNavigation,multiline=_ref.multiline,validate=_ref.validate
switch(event.which){case 9:tabNavigation&&this.utils.stopDOMEvent(event)
break
case 13:this.utils.stopDOMEvent(event)}switch(event.which){case 9:if(!tabNavigation)break
this.blur()
...
maxLength=(null!=(_ref1=this.getOptions().validate)?null!=(_ref2=_ref1.rules)?_ref2.maxLength:void 0:void 0)||0
if(13===event.which||maxLength>0&&value.length===maxLength)return event.preventDefault()
if(0===value.length){this.unsetPlaceholder()
if(event.target!==this.getEditableElement())return this.focus()}}}
KDContentEditableView.prototype.paste=function(event){var text
event.preventDefault()
text=event.originalEvent.clipboardData.getData("text/plain")
return document.execCommand("insertText",!1,text)}
...
KDTokenizedInput.prototype.getValue=function(options){var node,nodeValue,value,_i,_len,_ref,_ref1
null==options&&(options={})
value=""
_ref=this.getEditableElement().childNodes
for(_i=0,_len=_ref.length;_len>_i;_i++){node=_ref[_i]
"div"===(null!=(_ref1=node.tagName)?_ref1.toLowerCase():void 0)&&(value+="\n")
nodeValue=this.getValueOfNode(node)
"\n"!==nodeValue&&(value+=nodeValue)}return value===this.getOptions().placeholder?"":value}
KDTokenizedInput.prototype.getValueOfNode=function(node){var value
value=""
switch(node.nodeType){case Node.TEXT_NODE:""!==node.textContent&&(value+=node.textContent)
break
case Node.ELEMENT_NODE:value+=this.getValueOfElement(node)}return value}
KDTokenizedInput.prototype.getValueOfElement=function(element){var child,key,tagName,value,_i,_len,_ref,_ref1
key=null!=(_ref=element.dataset)?_ref.key:void 0
key&&(value=this.getValueOfTokenElement(key))
if(value)return value
tagName=element.tagName.toLowerCase()
switch(tagName){case"br":return"\n"
default:value=""
_ref1=element.childNodes
for(_i=0,_len=_ref1.length;_len>_i;_i++){child=_ref1[_i]
value+=this.getValueOfNode(child)}return value||""}}
KDTokenizedInput.prototype.getValueOfTokenElement=function(key){var view
view=this.getTokenView(key)
return key&&view?view.encodeValue():void 0}
KDTokenizedInput.prototype.getTokens=function(){return this.findTokensInElement(this.getEditableElement())}
KDTokenizedInput.prototype.findTokensInElement=function(element){var child,data,key,tokens,type,view,_i,_len,_ref,_ref1
tokens=[]
_ref=element.childNodes
for(_i=0,_len=_ref.length;_len>_i;_i++){child=_ref[_i]
switch(child.nodeType){case Node.ELEMENT_NODE:if(key=null!=(_ref1=child.dataset)?_ref1.key:void 0){view=this.getTokenView(key)
type=view.getOptions().type
data=view.getData()
tokens.push({type:type,data:data})}else tokens=tokens.concat(this.findTokensInElement(child))}}return tokens}
KDTokenizedInput.prototype.getTokenView=function(key){return this.tokenViews[key]}
KDTokenizedInput.prototype.matchPrefix=function(){var char,name,node,range,rule,start,_ref,_ref1,_results
if(!this.tokenInput&&(range=this.utils.getSelectionRange())){node=range.commonAncestorContainer
if(1===(null!=(_ref=node.children)?_ref.length:void 0))return node.textContent===node.children[0].textContent
start=range.startOffset-1
char=node.textContent[start]
_ref1=this.getOptions().rules
_results=[]
for(name in _ref1){rule=_ref1[name]
...
if(!this.tokenInput.parentNode)return this.cancel()
this.sanitizeInput()
...
KDTokenizedInput.prototype.showMenu=function(options,data){var pos,_ref
...
KDTokenizedInput.prototype.menuItemClicked=function(item){this.addToken(item.data)
return this.hideMenu()}
KDTokenizedInput.prototype.addToken=function(item,tokenViewClass){var pistachio,prefix,tokenKey,tokenView,type,_ref
null==tokenViewClass&&(tokenViewClass=this.getOptions().tokenViewClass)
...
this.tokenInput.parentElement.insertBefore(tokenView.getElement(),this.tokenInput)
...
if(this.tokenInput.parentNode){text=document.createTextNode(this.tokenInput.textContent)
this.getEditableElement().insertBefore(text,this.tokenInput)
this.tokenInput.nextSibling.remove()
this.tokenInput.remove()
this.utils.selectEnd(text)}return this.hideMenu()}
...
KDModalView.prototype.setButtons=function(buttonDataSet,destroyExists){var button,buttonOptions,buttonTitle,defaultFocusTitle,focused,_ref
...
buttonOptions.focus&&(focused=!0)}return!focused&&defaultFocusTitle?null!=(_ref=this.buttons[defaultFocusTitle])?_ref.setFocus():void 0:void 0}
...
KDTooltip.prototype.hide=function(){if(this.visible){KDTooltip.__super__.hide.apply(this,arguments)
...
//@ sourceMappingURL=/js/kd.0.0.1.js.map