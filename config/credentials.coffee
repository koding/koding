module.exports = (options) ->
    ...
    mongo = "#{options.boot2dockerbox}:27017/koding"
    redis =
      host: "#{options.boot2dockerbox}"
      port: "6379"
      db: 0
      url : "#{options.boot2dockerbox}:6379"
    rabbitmq =
      host: "#{options.boot2dockerbox}"
      port: 5672
      apiAddress: "#{options.boot2dockerbox}"
      apiPort: 15672
      login: "guest"
      componentUser: "guest"
      password: "guest"
    ...
    paymentwebhook =
      secretKey: "paymentwebhooksecretkey-dev"
    tokbox =
      apiKey: "45253342"
    ...
    odesk =
      ...
      version: "1.0"
      signature: "HMAC-SHA1"
      redirect_uri: "#{options.customDomain.host}:#{options.customDomain.port}/-/oauth/odesk/callback"
    facebook =
