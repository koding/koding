...
if cluster.isMaster
  cluster.fork() for i in [0..3]
  cluster.on "exit", (worker, code, signal) ->
    console.log "[LOADBALANCER] worker " + worker.process.pid + " died"
    cluster.fork()
  ...
  markServerAsDead = (port)->
    if port
      for server in addresses when server.port is port
        server.dead = yes
        server.good = no
        console.log "[LOADBALANCER] Webserver at port #{port} marked as dead."
  ...
  console.log "[LOADBALANCER] Running load balancer on port #{loadBalancerPort} pid:#{process.pid}"
