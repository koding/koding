    ...
    if messageData.name?
      if messageData.command is "first.start"
        ...
        switch messageData.command
          when 'add'
            @emit 'permissionToRun'
          when 'added.before' then console.log "[#{@worker.name} - #{@isoDateString()}] no permission to run. please start as 'force' or 'many' startmode."
      ...
      @connection.exchange "workerExchange", @getWorkerExchangeOptions(), (exchange) =>
        ...
        , (queue) =>
          ...
          queue.on "queueBindOk", =>
            unless @binary
              if @worker.message.option is "one" or "version"
                @startReporter "add"
              else
                @fetchWorkerExchange (workerExchange)=>
                  workerExchange.publish "input.worker", {worker: @worker},
