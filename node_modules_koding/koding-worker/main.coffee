...
class Worker extends EventEmitter
  ...
  defaultInit:(options, data)->
    ...
    @worker       =
      ...
      uuid      : @getUUID options.name, options.uuid
  ...
  # Create Uuid only once and store it in a file. If the worker restarts get
  # the uuid from the file next time instead of creating one.
  getUUID:(name, uuid) ->
    file = "kontrolUUID-" + name
    if fs.existsSync(file)
      uuid = (fs.readFileSync nodePath.join(__dirname, "../../#{file}"), 'utf-8').trim()
    else
      fs.writeFileSync nodePath.join(__dirname, "../../#{file}"), uuid, 'utf-8'
