...
function Connection (connectionArgs, options, readyCallback) {
  ...
  this.productName = (connectionArgs && connectionArgs.productName) || 'node-amqp'
...
function urlOptions(connectionString) {
  ...
  if (scheme != 'amqp' && scheme != 'amqps') {
    return emitError(emitter,'Connection URI must use amqp or amqps scheme. ' +
...
Connection.prototype._onMethod = function (channel, method, args) {
  ...
  if (channel > 0) {
    ...
    if (!this.channels[channel]._onChannelMethod) {
      return emitError(this, 'Channel ' + channel + ' has no _onChannelMethod method.');
    }
  ...
  switch (method) {
    ...
    default:
      return emitError(this, "Uncaught method '" + method.name + "' with args " +
          JSON.stringify(args));
  }
};
...
Connection.prototype.exchange = function (name, options, openCallback) {
  if (name == null) name = this.implOptions.defaultExchangeName;

  if (!options) options = {};
  if (name != '' && options.type == null) options.type = 'topic';
...
function Queue (connection, channel, name, options, callback) {
  ...
  self.name = name;
  self.consumerTagListeners = {};
  self.consumerTagOptions = {};
  ...
  self.options = { autoDelete: true, closeChannelOnUnsubscribe: false };
...
Queue.prototype.subscribeRaw = function (/* options, messageListener */) {
  ...
  var productName = this.connection.productName || 'node-amqp';
  var consumerTag = productName+'-'+process.pid+'-'+Math.random();
...
Queue.prototype.subscribe = function (/* options, messageListener */) {
  // TODO: this method is a bloody mess. Take some pride in your work, people!
  var self = this;
...
function mangleName(name) { return '~' + name; }
