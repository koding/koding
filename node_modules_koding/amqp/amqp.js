...
function parseInteger (buffer, size, emitter) {
  ...
  switch (size) {
    ...
    default:
      emitter.emit('error', "cannot parse ints of that size");
  }
}
...
function serializeInt (b, size, int, emitter) {
  if (b.used + size > b.length) {
    emitter.emit("error", "write out of bounds");
  }
...
function serializeShortString (b, string, emitter) {
  if (typeof(string) != "string") {
    emitter.emit("error","param must be a string");
  }
  var byteLength = Buffer.byteLength(string, 'utf8');
  if (byteLength > 0xFF) {
    emitter.emit("error","String too long for 'shortstr' parameter");
  }
  if (1 + byteLength + b.used >= b.length) {
    emitter.emit("error","Not enough space in buffer for 'shortstr'");
  }
  b[b.used++] = byteLength;
...
function serializationError(emitter, message) {
...
function serializeFields (buffer, fields, args, strict, emitter) {
  ...
  for (var i = 0; i < fields.length; i++) {
    ...
    if (!(field.name in args)) {
      if (strict) {
        serializationError("Missing field '" + field.name + "' of type '" + domain + "' while executing AMQP method '" + arguments.callee.caller.arguments[1].name + "'");
      }
      continue;
    }
    ...
    switch (domain) {
      case 'bit':
        if (typeof(param) != "boolean") {
          serializationError(emitter, "Unmatched field " + JSON.stringify(field));
        }
      ...
      default:
        serializationError(emitter, "Unknown domain value type " + domain);
    }
  }
}
...
function urlOptions(connectionString, emitter) {
  ...
  if (scheme != 'amqp' && scheme != 'amqps') {
    emitter.emit('Connection URI must use amqp or amqps scheme. ' +
...
Connection.prototype._onMethod = function (channel, method, args) {
  ...
  if (channel > 0) {
    ...
    if (!this.channels[channel]._onChannelMethod) {
      this.emit('error', 'Channel ' + channel + ' has no _onChannelMethod method.');
    }
  ...
  switch (method) {
    ...
    default:
      this.emit('error', "Uncaught method '" + method.name + "' with args " +
          JSON.stringify(args));
  }
};
...
// - appId
// - clusterId
function sendHeader (connection, channel, size, properties, emitter) {
  ...
  serializeInt(b, 2, channel, emitter);

  var lengthStart = b.used;

  serializeInt(b, 4, 0 /*dummy*/, emitter); // length

  var bodyStart = b.used;

  // HEADER'S BODY

  serializeInt(b, 2, classInfo.index, emitter);   // class 60 for Basic
  serializeInt(b, 2, 0, emitter);                 // weight, always 0 for rabbitmq
  serializeInt(b, 8, size, emitter);              // byte size of body
  ...
  serializeInt(b, 2, propertyFlags, emitter);
  // now the actual properties.
  serializeFields(b, classInfo.fields, props, false, this);
  ...
  b.used = lengthStart;
  serializeInt(b, 4, bodyEnd - bodyStart, emitter);
  b.used = bodyEnd;
...
Connection.prototype._sendBody = function (channel, body, properties) {
  ...
  if (typeof(body) == 'string') {
    ...
    sendHeader(this, channel, length, properties, this);
    ...
    sendHeader(this, channel, body.length, properties, this);

    var b = new Buffer(7);
    b.used = 0;
    b[b.used++] = 3; // constants.frameBody
    serializeInt(b, 2, channel, this);
    serializeInt(b, 4, body.length, this);
    this.write(b);
    this.write(body);
...
function Queue (connection, channel, name, options, callback) {
  ...
  this.options = { autoDelete: true, closeChannelOnUnsubscribe: true };
...
Queue.prototype.destroy = function (options) {
  ...
  .addCallback(closeChannelHelper(self));
};
