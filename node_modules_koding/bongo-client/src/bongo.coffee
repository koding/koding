...
class Bongo extends EventEmitter
  ...
  constructor:(options)->
    ...
    { @mq, @getSessionToken, @getUserArea, @fetchName, @resourceName,
      @precompiledApi, @apiEndpoint, @useWebsockets, @batchRequests } = options
    @useWebsockets ?= no
    @batchRequests ?= yes
    @getUserArea   ?= -> # noop
  ...
  guardMethod = (signatures, fn) ->
    signatures = signatures.map (signature) -> new Signature signature
    (rest...) ->
  ...
  wrapStaticMethods:do ->
    ...
    (constructor, constructorName, methods) ->
      ...
      (Object.keys methods).forEach (method) ->
        ...
        constructor[method] = guardMethod signatures, (rest...) ->
          rpc = {
            type: 'static'
            constructorName
            method
  ...
  wrapInstanceMethods:do ->
    ...
    (constructor, constructorName, methods) ->
      ...
      (Object.keys methods).forEach (method) ->
        signatures = methods[method]
        method += '_' if method in optimizeThese
        constructor::[method] = guardMethod signatures, ->
          id = @getId()
          data = @data unless id?
          rpc = {
            type: 'instance'
