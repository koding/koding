...
class Bongo extends EventEmitter
  ...
  constructor:(options)->
    EventEmitter this
    {@mq, @getSessionToken, @getUserArea, @fetchName, @resourceName} = options
    @getUserArea   ?= -> # noop
    ...
    @mq.on 'message', @bound 'handleRequestString'
    @mq.on 'broker.disconnected', @emit.bind this, 'disconnected'
    @opaqueTypes  = {}
  ...
  handleRequestString:(messageStr)->
    @handleRequest \
      try JSON.parse messageStr
      catch e then messageStr
  ...
  connect:(callback)->
    switch @readyState
      when 1 then return "already connected"
      when 3
        @readyState = 0
        @mq.connect => @connectHelper callback
      else
        @connectHelper callback

    if @mq.autoReconnect
      @mq.once 'disconnected', => @mq.on 'connected', => @connectHelper()

  disconnect:(shouldReconnect, callback)->
    if 'function' is typeof shouldReconnect
      callback = shouldReconnect
      shouldReconnect = no
    return "already disconnected"  if @readyState is 0
  ...
  send:(method, args)->
    ...
    throw new Error 'No channel!'  unless @channel
    scrubber = new Scrubber @localStore
    scrubber.scrub args, =>
  ...
  subscribe:(name, options={}, callback)->
    ...
    channel.setAuthenticationInfo
      ...
      name        : name
      clientId    : @getSessionToken()
    if callback? then channel.on 'broker.subscribed', -> callback channel
    return channel
