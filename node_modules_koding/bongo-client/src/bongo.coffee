...
class Bongo extends EventEmitter
  ...
  constructor:(options)->
    ...
    {@mq, @getSessionToken, @getUserArea, @fetchName, @resourceName, @precompiledApi, @apiEndpoint} = options
  ...
  ping:(callback)->
    @send 'ping', callback  if @readyState is CONNECTED
  ...
  sendXhr: (url, method, body) ->
    xhr = new XMLHttpRequest
    xhr.open method, url
    xhr.setRequestHeader "Content-type", "application/json;charset=UTF-8"
    xhr.onreadystatechange = =>
      if xhr.status is 0 or xhr.status >= 400
        @emit 'error', new Error "Error: #{ xhr.status }"

      return  if xhr.readyState isnt 4
      return  if xhr.status not in [200, 304]

      @handleRequest JSON.parse xhr.response
    messageString = JSON.stringify  { @channelName, body }
  ...
  handshakeDone:->
    @api or= @createRemoteApiShims REMOTE_API
    @readyState = CONNECTED
    @emit 'ready'
    @authenticateUser()
