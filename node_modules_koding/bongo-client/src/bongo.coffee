...
class Bongo extends EventEmitter
      ...
      (Object.keys methods).forEach (method) ->
        ...
        wrapper = guardMethod signatures, (rest...) ->
          rpc = {
            ...
            constructorName
            method
            # byPassBatching: contructor.byPassBatching
          }
          bongo.send rpc, rest
  ...
  sendHelper: (message) ->
    if @useWebsockets
      ...
      # if message.method?.bypassBatching
      #   bypassBatching = message.method.bypassBatching
      #   delete message.method.bypassBatching

      # do not batch social api requests
      byPassBatching = no
      if message.method?.constructorName in ["SocialMessage", "SocialChannel"]
        byPassBatching = yes
      if @batchRequests and not byPassBatching
