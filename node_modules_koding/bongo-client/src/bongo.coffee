...
class Bongo extends EventEmitter
  ...
  constructor:(options)->
    EventEmitter this
    {@mq, @getSessionToken, @getUserArea, @fetchName, @resourceName, @precompiledApi} = options
    @getUserArea   ?= -> # noop
  ...
  revive:(obj)->
    ...
    new Traverse(obj).map (node)->
      if node?.bongo_?
        ...
        @update KD.utils.xssEncode node
      else
        @update node
  ...
  connect:(callback)->
    ...
    if @mq.autoReconnect
      @mq.once 'disconnected', => @mq.on 'connected', => @reconnectHelper()
