...
class Client extends EventEmitter
  ...
  connect: (callback=noop)->
    ...
    @ws.on 'message', (data, flags)->
      ...
      if client.api.propertyIsEnumerable(scrubbed.method) and 'function' is typeof client.api[scrubbed.method]
        ...
        # Store.get return a wrapped function, then we cant access the real callbacks properties
        # so we re-get the function
        if callback?
          ourCallback = client.localStore.items[scrubbed.method]
          if ourCallback.timedout? and ourCallback.timedout
            logger.warn "callback received a response, but we already gave a timeout for this function, so dropping the message"
            return
          else
            if ourCallback.timeout?
              ourCallback.callTime = new Date
              logger.info "callback called in - #{ourCallback.callTime - ourCallback.startTime} msecs"
  ...
  # call remote kite's function (method) with these args
  # will call callback with Request instance
  #
  # @method   - name of the remote function
  # @args     - arguments array
  # @callback - callback function
  # @timeout  - will call callback function with a new KiteError if we dont receive,
  #             response in a timely fashion. timeout is in miliseconds, 5000 for 5 seconds
  callRemote: (method, args, callback=noop, timeout=1000)=>
    if callback and timeout
      @waitingCallbacks  = []
      callback.startTime = new Date
      callback.timeout   = timeout
      @waitingCallbacks.push callback
      # now setting timeout check loop
      if not @timeoutLoop?
        @timeoutLoop = setInterval ()=>
          removeFromWaitingList = []
          for fn in @waitingCallbacks
            if not fn.callTime
              if fn.timeout < (new Date) - fn.startTime
                logger.error "we didnt receive response for this function in time", fn.timeout, (new Date) - fn.startTime
                options =
                  remoteKite: this,
                  args: [new KiteError "request timeout", null]
                request = new Request options
                fn.timedout = true
                fn.callTime = 1
                fn.apply this, [request]
          @waitingCallbacks = (g for g in @waitingCallbacks when not g.callTime)
        , 0
    work = ()=>
  ...
  getKites: (query={}, eventListener, callback=noop)->
    ...
    @callRemote "getKites",
      ...
      (request)->
        ...
        if err
          return logger.error "error in getKites #{err}"
        clients = []
