...
class KiteConnectionWrapper extends EventEmitter
  ...
  callRemote: (method, args, authentication, callback=noop)=>
    ...
    scrubber.scrub [payload, callback], =>
      ...
      console.log "sending to connected client: #{JSON.stringify scrubbed}"
...
class Worker extends EventEmitter
  ...
  #   console.log ">>> sending to: ", client._socket._handle.fd
  ...
  callRemote: (client, method, args, callback=noop)=>
    ...
    options.kite =
      ...
      publicIP: @host # <-- we dont have a listening ip/port for a client
      port: @port.toString() # <-- port should be string, we dont need a listening ip and port for a client
  ...
  runServer: (@host="localhost", @port=9999)->
    ...
    @ws.on 'connection', (client)=>
      ...
      do (client)->
        console.log ">>> connected : ", client._socket._handle.fd
        client.on 'message', (data)=>
          scrubbed = JSON.parse data
          scrubber = new Scrubber worker.localStore
          unscrubbed = scrubber.unscrub scrubbed, (callbackId)->
            console.log ">>> callbackid : ", client._socket._handle.fd, callbackId
