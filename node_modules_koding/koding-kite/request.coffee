...
class AuthenticateFilter extends RequestFilter
  run: (request, callback)->
    ...
    if not authenticator
      return callback new KiteError("wrong/unsupported authentication type request: #{request.Authentication?.type}"), request

    authenticated = authenticator.authenticate(request.authentication.key, @worker.settings.kodingKey(), session)
