...
class Request
  ...
  getArgs: ()->
    ...
    if typeof(@args[0]) is "object" and @args[0].error?
      return error: new KiteError @args[0].error, data: @args[0].result
    else
      if @args[0].result?
...
class AuthenticateFilter extends RequestFilter
  run: (request, callback)->
    ...
    if not authenticator
      return callback new KiteError("wrong/unsupported authentication type: #{request.Authentication?.type}"), request

    authenticated = authenticator.authenticate(request.authentication.key, request, @worker)
    if not authenticated
      return callback new KiteError("could not authenticate request"), request
