...
class KiteConnectionWrapper extends EventEmitter
  ...
  go: (method, args, callback=noop)=>
    ...
    payload.kite =
      name: @kite.kitename
      username: @kite.environment.userName()
      id: @kite.uuid
      environment: @kite.environment.environment()
      region: @kite.environment.region()
      version: @kite.environment.version()
      hostname: @kite.environment.hostname()
      publicIP: ""
      port: @kite.port.toString()
    payload.withArgs = args
    payload.authentication =
      type: "kodingKey",
      key: @kite.environment.kodingKey()
...
class Kite extends EventEmitter
  ...
  start: (callback)->
    # TODO:
    @Kontrol = new Kontrol name: @kiteName, port: @port, environment: @environment, Visibility: @options.Visibility, ()=>
      @Kontrol.tell "register", null, (request)->
        [err, data] = request.Args
        if err
          logger.error "Cannot register to Kontrol", err
        else
          logger.info "Registered to kontrol", data
      callback null, this
