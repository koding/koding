...
class RemoteKite extends EventEmitter
  constructor: (@options={})->
    ...
    @options.kiteName        or= "application"
    @options.port            or= ""
    @options.settings        or= new Settings
    @options.api             or= {}

    @settings       = @options.settings
    @kodingKey      = @settings.readKeyFile()
    ...
    if @options.publicIP? and @options.port?
      ...
      @remoteAddr = @settings.kontrolAddress()

    if @constructor.name isnt 'Kontrol'
      @kontrol = new Kontrol settings: @settings, ->
  ...
  onMessage: (data, flags)=>
    ...
    if callback
        ...
        if syncCallback
          ...
          callback.apply this, [request, unscrubbed[1]]
    else
