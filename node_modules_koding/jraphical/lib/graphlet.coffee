...
module.exports = future class Graphlet
  ...
  abstractly: future (pipeline, method, arg, next)->
    last = pipeline.last()
    unless nodes = last.nodes or nodes = last.edges
  ...
  edges: future (pipeline, options, next)->
    ...
    Relationship.some selector, {limit, sort, skip, hint}, (err, relationships)->
      if err
        next err
      else
        if andNext
  ...
  nodes: future (pipeline, options, next)->
    ...
    if Array.isArray options
      ...
      if last.edges
        ...
        unless Object.keys(targets).length
          next()
        else
          for own targetName, targetIds of targets
