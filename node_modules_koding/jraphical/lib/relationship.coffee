...
module.exports = class Relationship extends Model
  ...
  save:(callback)->
    save_.call this, (err, model)=>
      ...
      if isEmitRequired @sourceName, @as
  ...
  @remove =(rest..., callback)->
    selector = rest.first
    Relationship.all selector, (err, res)=>
      @oldStaticRemove rest..., (err)->
        return callback err if err
        if res.length > 0
          for rel in res
            if isEmitRequired rel.sourceName, rel.as
              @constructor.emit "RelationshipRemoved", rel
        callback null

  update_ = @::update
  update:(rest..., callback)->
    update_.call this, rest..., (err)=>
      return callback err if err
      if isEmitRequired @sourceName, @as
  ...
  @update =(rest..., callback)->
    selector = rest.first
    Relationship.all selector, (err, res)=>
      @oldStaticUpdate rest..., (err)->
        return callback err if err
        if res.length > 0
          for rel in res
            if isEmitRequired rel.sourceName, rel.as
              @constructor.emit "RelationshipUpdated", rel
        callback null

  isEmitRequired = (sourceName, as)->
    sourceConstructor = Base.constructors[sourceName]
    relEvents = sourceConstructor?.broadcastableRelationships
    return relEvents and (as in relEvents)
