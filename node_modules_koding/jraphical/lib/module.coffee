...
module.exports = class Module extends Model
          ...
          cursor.toArray (err, docs)->
            if err
              ...
              , ->
                if limit
                  [results] = results
                else if not targetOptions.sort?
                  sortedResults = []
  ...
  getAdder_ =(constructor, name, def)->
    (target, options, callback)->
      ...
      unless target.getId()
        return callback new Error 'No target id!'

      Relationship.one rdef, (err, rel)->

        return callback err             if err
        return callback null, [rel], 1  if rel
  ...
  @remove =(selector, callback)->
    @all selector, (err, models)=>
      return callback err if err
      @oldStaticRemove selector, (err)=>
        return callback err if err
        if models.length > 0
          for model in models
            model.constructor.emit "RemovedFromCollection", model
        callback err
