class Pistachio
  ...
  addSymbolInternal: (symbol) ->
    ...
    @symbolsByDataPath[dataPath] = symbol         for dataPath in dataPaths
    @symbolsBySubViewName[subViewName] = symbol   for subViewName in subViewNames
  ...
  refreshChildren: (childType, items, forEach = (->)) ->
    ...
    symbolKey = switch childType
      when 'subview' then 'symbolsBySubViewName'
      when 'path'    then 'symbolsByDataPath'

    return  unless symbolKey?

    for item in items
      symbol = @[symbolKey][item]
