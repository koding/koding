    ...
    workQueue = queue.map (message, i) -> (fin) ->
      ...
      bongo.handleRequest channelName, { message, client }, (secretName, callbackId, args) ->
        ...
        bongo.scrubResponse callbackId, args, (message) ->
          console.log "scrubResponse", i, message
          alreadyCalled = responses[i]?
          responses[i] = message
          fin() unless alreadyCalled

    parallel workQueue, -> callback responses

  (req, res, next) ->

    { sessionToken, userArea, queue } = req.body
    console.log "sessionToken and userArea", sessionToken, userArea
