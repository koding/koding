class Signature
  ...
  constructor: (@paramTypes) ->
    [ instance, @key ] = Signature.getInstance @paramTypes

    return instance  if instance?

    rest = no
    
    for type, i in @paramTypes when type.isRest is yes
  ...
  @poolKey = (paramTypes) -> paramTypes
    .map (paramType) ->
      { name, tag } = paramType
      return "[#{paramType[0].name}]"   if Array.isArray paramType
      return "Rest(#{tag.name})"        if paramType is Rest
      return name
    .join ','

  @getInstance = (paramTypes) ->
