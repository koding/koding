...
module.exports = class Model extends Base
  ...
  @setClient =(overload, shouldConnect=yes)->
    ...
    if overload instanceof SkinDb
      ...
      client = mongo.db overload, safe: no
  ...
  ###
  {@one, @all, @some, @someData, @remove, @removeById, @drop, @cursor
   @count, @teasers, @hose, @mapReduce, @assure, @update, @each} = require './find'
  ###
  Feeding
...
#            @update @node, yes
  ...
  remove:(callback)-> console.log @getId(); @constructor.remove _id: @getId(), callback
