...
module.exports = class Bongo extends EventEmitter
  ...
  constructor:(options)->
    {@mq, @mongo, @resourceName, @root, @fetchClient, @softDelete, models} = options
    @softDelete ?= no
    @setClient @mongo
  ...
  deleteInstance:(instance)->
    if @softDelete then _deleteSoftly instance
    else _deleteForReal instance

  handleEvent:(type, ctx, event, rest)->
    switch type
      when 'instance'
        return @deleteInstance ctx  if event is 'deleteInstance'
        @mq.emit 'updateInstances', getInstanceRoutingKey(ctx, event), (rest[0] or null), autoDelete: no
      when 'static'
        if event is 'global-listener-added'
          [actualEvent, listener] = rest
          #@mq.on 'event-'+ctx.name, actualEvent, listener]
        else if event is 'broadcast'
          [secretChannelId, message] = rest
          @fetchBrokerExchange (brokerExchange)->
            brokerExchange.publish secretChannelId, JSON.stringify message
        else
  ...
  invokeMethod:do->
    ...
    apply =(bongo, ctx, method, args, sessionToken, userArea)->
      ...
      bongo.handleError "unknown method! #{method}"  unless fn?
