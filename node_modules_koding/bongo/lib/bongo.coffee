...
module.exports = class Bongo extends EventEmitter
  ...
  constructor: (options) ->
    ...
    { @mq, @mongo, @resourceName, @root, @metrics, @redisClient
  ...
  registerModel: (path) ->
    ...
    if cons.name? and ('function' is typeof cons)
      ...
      for own name, konstructor of cons
        models.push @defineModel name, konstructor
  ...
  findCode = (paths) ->
    ...
    .filter (_path) -> return !(/\.test\.coffee$/.test _path)
  ...
  loadModels: ->
    ...
    Promise.map @modelIndexPaths, (modelPath, index) =>
      ...
      # console.log "Model list found:", modelPath
      # console.log "Loading #{Object.keys(cache).length} models..."

      Object.keys(cache).map (name) =>
        # console.log "Loading model:", name
        @registerModel path.join @modelPaths[index], cache[name]

    .catch (err) =>
      # console.log 'Cache not found or corrupted, re-building...', err.message
      @buildModelIndexes()

    .then =>
      # console.log "Ready!", (Object.keys @models).length
      @emit 'apiReady'
  ...
  buildModelIndexes: ->
    ...
    Promise.map @modelPaths, (_path, index) =>
      ...
      findCode([_path]).map (codePath) =>
        ...
        Promise.try =>
          model = @registerModel codePath
          if model and not Array.isArray model
            plainPath = codePath.replace _path, ''
            cache[model] = plainPath
  ...
  handleRequest: (secretName, request, callback) ->
    ...
    # Keep callId's in-memory and prevent calling the callback
    # more than once ~ GG
    CALLHISTORY = []
    ...
    unscrubbed = scrubber.unscrub message, (callbackId)=>
      ...
      unless @remoteStore.has storeKey
        fn =
          if callback?
          then (args...) ->
            if (CALLHISTORY.indexOf callbackId) > -1
              console.log '[Bongo:LEAKED_CALL]', methodName
            else
              CALLHISTORY.push callbackId
