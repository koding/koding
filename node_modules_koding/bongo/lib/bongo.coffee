...
module.exports = class Bongo extends EventEmitter
  ...
  constructor:(options)->
    { @mq, @mongo, @resourceName, @root, @metrics
    ...
    # TODO comment out this when we completely remove broker
    #@initMQ()
  ...
  initMQ: ->
    ...
    @connection = amqp.createConnection @mqConfig, reconnect: no
    @connection.setMaxListeners(0)
    @readyState = 0
    @connection.once 'ready', => @readyState = 1
  ...
  findCode = (paths) ->
    ...
    .filter (path) -> return /(\.coffee|\.js)$/.test(path)
    # if test file return false and exclude file
    .filter (path) -> return !isTestFile path
  ...
  isTestFile = (path) ->
    ...
    return endsWith path, 'test.coffee'
  ...
  loadConstructors: (consPaths) ->
    ...
    findCode(paths).map (codePath) =>
      ...
      .catch (err) =>
        @emit "error", new Error "Error loading a class: #{ codePath } #{ err.message }"
    .then =>
      @emit 'apiReady'
  ...
  publishEventToExchange: (data) ->
    ...
    @mq.connection?.exchange "#{exchangeName}", EXCHANGE_OPTIONS, (exchange) =>
      ...
      exchange.publish "", message, type: type
      exchange.close()
  ...
  invokeMethod: do ->
    ...
    apply = (bongo, ctx, method, args, sessionToken, userArea, client) ->
      ...
      unless ctx?
        bongo.handleUserError "bad instance! #{method.method}"
        return
      if 'function' is typeof method.method
        fn = method.method
      else if ctx? and 'string' is typeof method.method
        ...
        fn = ctx["#{method.method}$"] ? ctx[method.method]
      unless fn?
        bongo.handleUserError "unknown method! #{method.method}"
        return


      applyWithMerics = (fn, ctx, args) ->
        if bongo.metrics
          # wrapping callback function if there is one in the args
          if typeof (callback = args[args.length - 1]) is 'function'
            start = new Date()
            args[args.length - 1] = (rest...) ->
              duration = new Date() - start
              bongo.metrics.sendMethodMetrics { method, duration }
              callback rest...
          # else sending metrics with duration equal to zero
          else
            bongo.metrics.sendMethodMetrics { method, 0 }
        fn.apply ctx, args
        ...
        switch method.type
          when 'static'
            if konstructor.hasSharedMethod method.method
              apply this, konstructor, method, args, sessionToken, userArea, client
            else
              ...
              if method.id
                konstructor.one? _id: method.id, (err, instance)=>
                  if err
                    ...
                    apply this, instance, method, args, sessionToken, userArea, client
              else if method.data
