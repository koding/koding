...
module.exports = class Bongo extends EventEmitter
  ...
  constructor:(options)->
    {@mq, @mongo, @resourceName, @root, @fetchClient, @verbose, models, @kite, @mqConfig} = options
  ...
  publishEventToExchange: (exchangeEvent, data) ->

    {exchangeName} = @mqConfig

    # TODO exchange version must be innjected here, when we have that support
    @mq.connection?.exchange "#{exchangeName}:0", EXCHANGE_OPTIONS, (exchange) =>
      {eventName, body, account} = data
      message = {
        account
        eventName: "social"
        body:
      ...
      exchange.publish "", message, type: "dispatcher_notify_user"
      exchange.close()
