...
module.exports = class Bongo extends EventEmitter
  ...
  findCode = (paths) ->
    ...
    .reduce((a, b) ->
      a.concat b
    , [])
    # only consider .js and .coffee files as "code"
  ...
  loadConstructors: (consPaths) ->
    ...
    .catch (err) ->
      throw new Error "Error loading a class: #{ err.message }"
  ...
  invokeMethod:do->
    ...
    (method, args, secretName, sessionToken, userArea)->
      unless method?
        ...
        switch method.type
          when 'static'
            if konstructor.hasSharedMethod method.method
              apply this, konstructor, method.method, args, sessionToken, userArea
            else
              ...
              if method.id
                konstructor.one? _id: method.id, (err, instance)=>
                  if err
                    ...
                    apply this, instance, method.method, args, sessionToken, userArea
              else if method.data
        ...
        if kallback?
          apply this, null, kallback, args, sessionToken, userArea
        else
          @handleError 'Unknown method type!'
  ...
  xhrHandshake: (callback) ->
    if @precompiledApi
      callback()
    else
      @describeApi (api) -> callback api
