...
module.exports = class Bongo extends EventEmitter
  ...
  parseRoutingKey: do ->
    getEdgeMeaning = (i)->
      switch i
        ...
        when 2 then 'event'
        when 3 then 'secretChannelId'
        else        "additionalProperty#{i-3}"
  ...
  respondToClient:(routingKey, message)->
    ...
    @fetchBrokerExchange (brokerExchange)->
      console.log {brokerExchange, brokerExchange}
        ...
        @mq.connection.queue  @resourceName, (@queue)=>
          ...
          queue.on 'queueBindOk', =>
            queue.subscribe((message, headers, deliveryInfo)=>
              ...
              msg = ''+message?.data
              routingInfo = @parseRoutingKey routingKey
              switch routingInfo.event
                when 'message'
                  @handleRequest routingKey, msg
                when 'join'
                  @describeApi (api)=>
                    message =
                      method    : 'defineApi'
