...
module.exports = class Bongo extends EventEmitter
  ...
  handleRequest:(secretName, request, callback)->
    ...
    if @metrics
      # wrapping callback function if there is one in the scrubbed args
      if typeof (cb = unscrubbed[unscrubbed.length - 1]) is 'function'
        unscrubbed[unscrubbed.length - 1] = (rest...) =>
          duration = new Date() - start
          @metrics.sendMethodMetrics { method, duration }
          cb rest...
      # else sending metrics with duration equal to zero
      else
        @metrics.sendMethodMetrics { method, 0 }
