...
class Processes extends EventEmitter
  ...
  run: (options,callback) ->
    ...
    if options.kontrol?.enabled
      console.log "[PROCESSES][#{name}] kontrold enabled"
      @kontroldProcess options
    else
      console.log "[PROCESSES][#{name}] kontrold disabled"
  ...
  kontroldProcess: (options) ->
    {name,onExit,restart,restartInterval,verbose} = options
    ...
    @workerProcess[name].on 'permissionToRun', =>
      ...
      unless @list[name]?.name is name
          ...
          @workerProcess[name].startReporter()
          ...
          @workerProcess[name].fetchWorkerExchange (workerExchange) =>
            ...
            @workerProcess[name].worker.pid = @child.pid
    ...
    ### Stop  ###
    #############
    @workerProcess[name].on 'stopRequest', =>
    ...
    @workerProcess[name].on 'killWorkerProcess', =>
      ...
      @workerProcess[name].worker.status = 4
    ...
    ### Check ###
    #############
    @workerProcess[name].on 'checkProcess', =>
    ...
    @on 'monitorData', (data) =>
      ...
      data['uuid'] = @workerProcess[name].worker.uuid
      @workerProcess[name].fetchWorkerExchange (workerExchange) =>
        @workerProcess[name].worker.message.command = "ack"
    ...
    process.on 'SIGINT', =>
      @workerProcess[name].worker.status = 5
      @workerProcess[name].fetchWorkerExchange (workerExchange) =>
        @workerProcess[name].worker.message.command = "ack"
        workerExchange.publish "input.worker", {worker: @workerProcess[name].worker},
          appId: "#{@workerProcess[name].worker.uuid}"
  ...
  populateChildObject: (options, @child) ->
    ...
    @child.on 'exit', ()=>
      ...
      if options.kontrol?.enabled
        ...
        @workerProcess[name].stopReporter()
        @workerProcess[name].worker.status = 4
        @workerProcess[name].fetchWorkerExchange (workerExchange) =>
          @workerProcess[name].worker.message.command = "ack"
          workerExchange.publish "input.worker", {worker: @workerProcess[name].worker},
