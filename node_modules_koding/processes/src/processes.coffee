...
class Processes extends EventEmitter
  ...
  run: (options,callback) ->
    {name,restartInterval,verbose,port} = options

    name            ?= Date.now()
    restartInterval ?= 1000
    verbose         ?= no

    if options.kontrolEnabled
      console.log "[PROCESSES][#{name}] kontrold authorization enabled" if verbose
  ...
  startProcess: (options) ->
    {name,opts,cmd,stdout,stderr,port} = options
  ...
  kontroldProcess: (options) ->
    {name,restart,verbose,port} = options

    console.log "[PROCESSES][#{@name}] force mode enabled. Trying to stop and kill all other #{name} workers" if verbose and options.kontrolForceStart
    ...
    workerProcess = new Worker {
      ...
      name               : options.name
      uuid               : hat()
      option             : if options.kontrolForceStart then 'force' else ''
    ...
    workerProcess.on 'permissionToRun', =>
      ...
      unless @list[name]?.name is name
          ...
          workerProcess.fetchWorkerExchange (workerExchange) =>
            ...
            workerProcess.worker.port = @child.port
  ...
  populateChildObject: (options, @child) ->
    {name,onMessage,modulePath,args,restart,restartInterval,cmd,log,onExit,stdout,stderr,verbose,die,port} = options
    onExit ?= ->
    ...
    @child.on 'message',(msg) =>
      ...
      else if options.kontrolEnabled and options.kontrolNodeProcess
        @emit "childData", msg
