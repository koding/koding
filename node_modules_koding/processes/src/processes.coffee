...
{mq, kontrold}      = require 'koding-config-manager'
...
module.exports = class Processes extends EventEmitter
  ...
  run: (cmd, callback=->)->
    ...
    # sp = require('child_process').spawn("bash -c", cmd.split(" "))
    # sp.stdout.on "data", (data) ->
    #   console.log "[#{cmd}]"+(data+"").replace("\n","")
    # sp.stderr.on "data", (data) -> console.log "[#{cmd}]"+(data+"").replace("\n","")
    # # sp.on "close", (code) -> callback sp.pid
    # sp.on "exit", (code) -> callback sp.pid

    ch = require('child_process').exec cmd,callback # cmdA is split wrongly when quotation marks are involved
  ...
  startProcess: (options) ->
    { name, opts, cmd, stdout, stderr, verbose } = options
  ...
  populateChildObject: (options, child) ->
    ...
    child.on 'close', =>
      ...
      if restart is yes
        ...
        wait restartTimeout, => @_run options
