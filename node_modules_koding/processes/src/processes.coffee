...
class Processes extends EventEmitter
  ...
  kontroldProcess: (options) ->
    ...
    console.log "[PROCESSES][#{@name}] force mode enabled. Trying to stop and kill all other #{name} workers" if verbose and options.forceStart
    ...
    workerProcess = new Worker {
      ...
      name               : options.name
      uuid               : hat()
      option             : if options.forceStart then 'force' else ''
  ...
  populateChildObject: (options, @child) ->
    ...
    @child.on 'message',(msg) =>
      ...
      else if options.needPermission and options.isWorker
        @emit "childData", msg
