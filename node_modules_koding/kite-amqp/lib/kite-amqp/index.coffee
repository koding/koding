...
class Kommunicator extends EventEmitter
  ...
  connectToMessageBus: ()=>
    ...
    @mergen.subscribe "#{@publicKey}-kite-all"
    @mergen.subscribe "#{@publicKey}-kite-#{@serviceGenericName}"
    @mergen.subscribe "#{@publicKey}-kite-#{@serviceGenericName}-#{@serviceUniqueName}"
    ...
    @mergen.on "message", (channel, message)=>
      ...
      try
        k = message.split(' ')
        cmds[k[0]] k[1...], (ret)=>
          @mergenData.publish "kite-#{k[k.length-1]}", "socketwrite " + ret 
      catch Error
        console.trace Error
