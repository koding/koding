...
class Kommunicator extends EventEmitter
  ...
  constructor:(@kite, @publicKey, @serviceGenericName, @serviceUniqueName, @api, @host='localhost', @port=6380)->
    ...
    @kiteList = {}
    @commands = 
      'kite.who': (options, callback)->
        callback false, @getChannelNameForKite()

      'kite.join': (options, callback)->
  ...
  receivedCommand: (from, cmdId, cmd, args)->
    console.log "receivedCommand", arguments
    if cmd.indexOf('reply-') > -1
      cmdId = cmd.split('reply-')[1]
      #console.log ">>>> reply cmd", cmd, args
      @waitingReplies[cmdId]?.call this, false, {args:args, from:from}
      return

    fn = @commands[cmd]
    if fn
      ret = fn.call this, {args:args, from:from}, (err, result)=>
        @sendCommand(from, "reply-#{cmdId}", [err, result])
      return

    if @api.propertyIsEnumerable(cmd) and 'function' is typeof @api[cmd]
      fn = @api[cmd]
      fn.call @kite, {args:args, from:from}, (err, result)=>
        @sendCommand(from, "reply-#{cmdId}", [err, result])
      return
  ...
  queueCommand:(shortChannelName, cmd, args, callback=null)->

    if not channel
      channel = @getChannelNameForGenericName()
    else
      channel = @getChannelNameForGenericName(shortChannelName)
    ...
    if callback
      @waitingReplies[@callbackId] = callback

    datastr = JSON.stringify(data)
    @mergenData.rpush channel, datastr, ()=>
      console.log "pushed to ", channel, datastr
      @sendCommand channel, "kite.consume", datastr

  sendCommand: (channel, cmd, args, callback=null)->
  ...
  connectToMessageBus: (callback)=>
    ...
    # mode
    @mergenData = redis.createClient(@port, @host, {socket_nodelay:false})
    ...
    @mergen = redis.createClient(@port, @host, {socket_nodelay:false})
    @mergen.on "error", (err)->
      console.log("Error ",err)

    channelNames = [@getChannelNameForAll(), @getChannelNameForGenericName(), @getChannelNameForKite()]

    for channel in channelNames
      @mergen.subscribe channel

    @mergen.on "message", (channel, message)=>
      message = JSON.parse message
      {from, id, cmd, args} = message
      @receivedCommand from, id, cmd, args

    # we have 3 channels to subscribe on startup,
    # we have to wait for all before we emit ready
    @mergen.on "subscribe", (channel, count)->
      channelNames = _.without(subscribeToChannelNames, channel)
...
module.exports = class Kite extends EventEmitter
  ...
  run:(@config, @key, @manifest)->
    ...
    hp = @config.messagebus.url.split(':')
    mergen_host = hp[0] 
    mergen_port = hp[1]
