...
class Kommunicator extends EventEmitter
  ...
  getGroupName:(groupName)->
    "#{@publicKey}-kite-kite-#{@publicKey}-Deployer"
  ...
  receivedCommand: (from, cmdid, cmd, args)->
    ...
    if @api.propertyIsEnumerable(cmd) and 'function' is typeof @api[cmd]
      fn = @api[cmd]
      ret = fn.apply @kite, args
      @sendCommand(from, "reply-#{cmdid}", ret)
      return

    @sendCommand(from, "reply-#{cmdid}", [false])
  ...
  queueCommand:(cmd, args, cb=false)->
    ...
    @mergenData.rpush "#{@getChannelNameForGenericName()}", data, ()=>
      @sendCommand @getChannelNameForGenericName(), "kite.consume", data
...
module.exports = class Kite extends EventEmitter
  ...
  group: (groupName, cmd, arg, callback)->
  ...
  one: (chanName, cmd, args, callback)->
    channame = chanName
    console.log "sending to channame", channame
  ...
  getKiteId:-> "#{process.pid}|#{os.hostname().replace /\./g, '_'}"
