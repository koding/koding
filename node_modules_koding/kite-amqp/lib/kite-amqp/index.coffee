...
class Kommunicator extends EventEmitter
    ...
    @commands = 
      ...
      'kite.consume': (opts, cb)->
        @mergenData.lpop @getChannelNameForGenericName(), (err, val)=>
          ...
          @receivedCommand val.from, null, val.cmd, val.args
  ...
  queueCommand:(cmd, args, cb=false)->
    ...
    @mergenData.rpush "#{@getChannelNameForGenericName()}", datastr, ()=>
      console.log "pushed to ", @getChannelNameForGenericName(), datastr
      @sendCommand @getChannelNameForGenericName(), "kite.consume", datastr
  ...
  connectToMessageBus: (cb)=>
    ...
    # coffeescript gives Users/aybarsbadur/ping.kite/index.coffee:105:53: error: unexpected CALL_END
    # if these are on one line idk why
    ...
    @sendCommand @getChannelNameForGenericName(), "kite.join", [@getChannelNameForGenericName(), @getChannelNameForKite()]
      ...
      # @queueCommand "hello", ["world"], (ret)->
      #   console.log "queue command returned"

    subscribeCheck = setInterval ()=>
      if subscribeToChannelNames.length is 0
        clearTimeout subscribeCheck
        cb()
    , 10
    @connected = true
...
module.exports = class Kite extends EventEmitter
  ...
  queue: (cmd, args, callback)->
    @communicator.queueCommand cmd, args, callback
