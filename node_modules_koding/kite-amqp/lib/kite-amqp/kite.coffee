{EventEmitter} = require 'kd-microemitter'
fs    = require "fs"
path  = require "path"
Kite  = require './index'
...
module.exports = class KiteBase extends EventEmitter
  ...
  constructor:(config, functions, roundRobin=false)->
    ...
    client.get url, (res) =>
      ...
      res.on "end", =>
        ...
        @kite = new Kite config.name, functions, roundRobin
        @kite.run @config
  ...
  @worker: (options, functions, roundRobin)=>
    options = @key options
    @instance or= new @ options, functions, roundRobin
  ...
  @key: (manifest)=>
    ...
    if fs.existsSync KODING_KEY
      try
        key = fs.readFileSync(KODING_KEY).toString()
        manifest.key = key
        manifest.username or= manifest.author
        manifest.owner or= manifest.username
        manifest.name = "#{key}-#{manifest.name}"
      catch error
        console.log error
        process.exit 1
    else
      console.log "You need to register this host using `kd register`"
      process.exit 1
    manifest
