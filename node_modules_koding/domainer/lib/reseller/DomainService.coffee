...
module.exports = class DomainService extends ServiceManager
  ...
  # This is for domai.nr's untrustable domain suggestions
  # It possibly returns .co domains as available even they are not. ~ GG
  # blackListedTLDs = ["co"]
  ...
  getDomainSuggestions: (domainName, callback) ->
    ...
    # Following is added for using Reseller's suggestion
    # Use it if domai.nr becames useless. ~ GG
    ...
    # @makeRequestToResellerApi options, (err, suggestionResponse)->
    #   return callback err, suggestions  if err

    #   suggestedNames = Object.keys(suggestionResponse)
    #   if suggestedNames.length is 0
    #     return callback null, suggestions

    #   for own domain, status of suggestionResponse
    #     if status[extension] is 'available'
    #       if extension in availableTLDs # and domain isnt domainName
    #         attachPrice {domain:"#{domain}.#{extension}", extension}
    #         foundASuggestion = yes
    ...
    # Domai.nr API Implementation
    # which return false availability most of the time
    ...
    #       if extension not in blackListedTLDs  \
    #          and result.domain isnt domainName \
    #          and extension in availableTLDs
