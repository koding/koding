module.exports = class Request
  ...
  request: ({ method, operation, httpOptions, data, paymentMethodId }, callback) ->
    ...
    log = (type, {paymentMethodId, data, operation}) ->
      loggableOperations = ["subscriptions", "transactions"] #it can be retrieved from config
      loggedOps = loggableOperations.join("|")

      if new RegExp("^(#{loggedOps})").test operation
    ...
    log "info", {paymentMethodId, operation, data}
      ...
      res.on 'end',  ->
        ...
        xml2js.parseString body, (err, result)=>
          if err
            ...
            log "error",{paymentMethodId, operation, data: err}
          else if result.error
            callback [new models.Error result.error.symbol[0], result.error.description[0]._]
            log "error", {paymentMethodId, operation, data: result.error}
            ...
            callback errors
            log "error", {paymentMethodId, operation, data: result.errors}
          else
            # Find next page's URL if necessary
            nextUrl = parseNextUrl res  if res.headers.link
            callback null, result, nextUrl
            log "info", {paymentMethodId, operation, data: JSON.stringify(result)}
