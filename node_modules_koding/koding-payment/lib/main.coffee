...
getPlanInfo = (data, callback)->
  recurlyGet "plans/#{data.planCode}", (err, res) ->
    if err
      callback err, {}
    else
      plan = []
      for page in res
        ...
        if page.plan
          plan =
            name  : page.plan.plan_code[0]
            title : page.plan.name[0]
            desc   : if page.plan.description[0].$ then "" else page.plan.description[0]
            amount : parseInt page.plan.unit_amount_in_cents[0]['USD'][0]['_']
            initial: parseInt page.plan.setup_fee_in_cents[0]['USD'][0]['_']
          callback no, plan
        else
          callback yes, {}
        break
...
addPlan = (data, callback) ->
  ...
  xml = """<plan>
             ...
             <name>#{data.planTitle}</name>
             <setup_fee_in_cents>
               <USD>#{parseInt(data.feeInitial) * 100}</USD>
             </setup_fee_in_cents>
             <unit_amount_in_cents>
               <USD>#{parseInt(data.feeMonthly) * 100}</USD>
             </unit_amount_in_cents>
  ...
  recurlyPost 'plans', xml, (err, res) ->
    if err
      ...
      plan =
        name  : res.plan.plan_code[0]
        title : res.plan.name[0]
        desc   : if res.plan.description[0].$ then "" else res.plan.description[0]
        amount : parseInt res.plan.unit_amount_in_cents[0]['USD'][0]['_']
        initial: parseInt res.plan.setup_fee_in_cents[0]['USD'][0]['_']
      callback no, plan

deletePlan = (data, callback) ->
  recurlyDel "plans/#{data.planCode}", (err, res) ->
    if err
      callback err, {}
    else
      callback no, res
