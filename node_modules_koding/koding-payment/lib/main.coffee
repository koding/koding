...
module.exports = class Recurly
      ...
      res.on 'end',  ->
        ...
        xml2js.parseString body, (err, result)->
          if err
            ...
            errors = []
            for error in result.errors.error
              if err['$']
                errors.push errorObj error['$']['symbol'], error['_'], error['$']['field']
              else
                errors.push errorObj error error
            callback errors
          else
  ...
  # POST request
  post = (operation, data, callback)-> request 'POST', operation, data, callback
  # PUT request
  put = (operation, data, callback)-> request 'PUT', operation, data, callback
  # DELETE request
  del = (operation, callback)       -> request 'DELETE', operation, '', callback
  ...
  planObj = (plan)->
    ...
    feeMonthly  : parseInt plan.unit_amount_in_cents[0]['USD'][0]['_']
    feeInitial  : parseInt plan.setup_fee_in_cents[0]['USD'][0]['_']
    feeInterval : parseInt plan.plan_interval_length[0]['_']
  ...
  @getSubscriptions: (accountCode, callback)->
    get "accounts/#{accountCode}/subscriptions", (err, res)->
      ...
      subs = []
      for page in res
        subs.push subObj s  for s in page.subscriptions?.subscription
  ...
  @getBilling: (accountCode, callback)->
    get "accounts/#{accountCode}/billing_info", (err, res)->
      callback err, unless err then billObj res.billing_info
