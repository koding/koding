        ...
        if page.plans?.plan
          for p in page.plans.plan
            plans.push
              ...
              feeInterval: parseInt p.plan_interval_length[0]['_']
      callback no, plans
      ...
      for page in res
        ...
        if page.plan
          plan =
            ...
            feeInterval: parseInt page.plan.plan_interval_length[0]['_']
          callback no, plan
        else
          callback yes, {}
        break
  ...
  recurlyPost 'plans', xml, (err, res) ->
    if err
      ...
      plan =
        ...
        feeInterval: parseInt res.plan.plan_interval_length[0]['_']
      callback no, plan
...
updatePlan = (data, callback) ->
  ...
  recurlyPut "plans/#{data.code}", xml, (err, res) ->
    if err
      callback err, {}
    else
      plan =
        ...
        if page.subscriptions?.subscription
          for s in page.subscriptions.subscription
            sub =
              ...
              amount   : s.unit_amount_in_cents[0]['_']
            if s.pending_subscription
              sub.pending =
                plan      : s.pending_subscription[0].plan[0].plan_code[0]
                quantity  : s.pending_subscription[0].quantity[0]['_']
            subs.push sub
      callback no, subs
      ...
      for page in res
        ...
        if page.subscription
          sub =
            ...
            amount   : page.subscription.unit_amount_in_cents[0]['_']
          if page.subscription.pending_subscription
            sub.pending =
              plan      : page.subscription.pending_subscription[0].plan[0].plan_code[0]
              quantity  : page.subscription.pending_subscription[0].quantity[0]['_']
          callback no, sub
        else
          callback yes, {}
  ...
  recurlyPost 'subscriptions', xml, (err, res) ->
    if err
      ...
      subs =
        ...
        amount     : res.subscription.unit_amount_in_cents[0]['_']
      callback no, subs

addSubscriptionAndAccount = (data, callback) ->
  data.quantity ?= 1
  ...
  xml = """<?xml version="1.0" encoding="UTF-8"?>
           <subscription>
             ...
             <quantity>#{data.quantity}</quantity>
             <account>
               <account_code>#{data.accountCode}</account_code>
               <username>#{data.username}</username>
               <email>#{data.email}</email>
               <first_name>#{escape data.firstName}</first_name>
               <last_name>#{escape data.lastName}</last_name>
               ...
               <billing_info>
                 ...
                 <first_name>#{escape data.cardFirstName}</first_name>
                 <last_name>#{escape data.cardLastName}</last_name>
                 <address1>#{escape data.address1}</address1>
                 <address2>#{escape data.address2}</address2>
                 <city>#{escape data.city}</city>
                 <state>#{escape data.state}</state>
                 <zip>#{data.zipCode}</zip>
                 <country>#{escape data.country}</country>
...
terminateUserSubscription = (accountCode, data, callback) ->
  data.refund ?= "none"
  recurlyPut "subscriptions/#{data.uuid}/terminate?refund=#{data.refund}", "", (err, res) ->
    if err
      callback err, {}
    else
      subs =
        ...
        if page.transactions?.transaction
          for t in page.transactions.transaction
            ...
            trans =
              ...
              source    : t.source[0]
            if t.invoice
              trans.invoice = t.invoice[0]['$']['href']
            if trans.status in ['success', 'void']
              trans.owner      = "#{unescape billing.first_name[0]} #{unescape billing.last_name[0]}"
              trans.card       = "#{billing.card_type[0]} - #{billing.first_six.toString().substring(0,4)}-...-#{billing.last_four}"
              trans.cardType   = billing.card_type[0]
              trans.cardNumber = "#{billing.first_six}...#{billing.last_four}"
...
getUserAdjustments = (accountCode, callback)->
   recurlyGet "accounts/#{accountCode}/adjustments", (err, res) ->
    if err
      callback err, []
    else
      adjustments = []
        ...
        if page.adjustments?.adjustment
          for t in page.adjustments.adjustment
            adjustments.push
              ...
              amount      : t.total_in_cents[0]['_']
              currency    : t.currency[0]
              description : if t.description[0].$ then "" else unescape t.description[0]
              type        : t['$']['type']
              origin      : t.origin[0]
    ...
    if err
      ...
      for page in res
        ...
        if page.billing_info
          ...
          billing =
            cardFirstName     : unescape info.first_name[0]
            cardLastName      : unescape info.last_name[0]
            ...
            address1          : unescape info.address1[0]
            address2          : if typeof info.address2[0] is "string" then unescape info.address2[0] else ""
            city              : if typeof info.city[0] is "string" then unescape info.city[0] else ""
            state             : if typeof info.state[0] is "string" then unescape info.state[0] else ""
            zip               : info.zip[0]
            country           : unescape info.country[0]
        else
          billing = {}
      callback no, billing
  ...
  recurlyGet "accounts/#{accountCode}", (err, res) ->
    if err
      ...
      for page in res
        ...
        if page.account
          account =
            firstName : unescape page.account.first_name[0]
            lastName  : unescape page.account.last_name[0]
...
setBilling = (accountCode, data, callback) ->
  ...
  recurlyPut "accounts/#{accountCode}/billing_info", xml, (err, res) ->
    if err
      callback err, {}
    else
      callback no, res

setAccountWithBilling = (accountCode, data, callback) ->
  # Remove credit card number if it's not updated
  if data.cardNumber? and data.cardNumber.indexOf('...') > -1 or data.cardCV? and data.cardCV == 'XXX'
    delete data.cardNumber
    delete data.cardCV
  ...
  xmlWithBilling = """<?xml version="1.0" encoding="UTF-8"?>
           <account>
             <account_code>#{accountCode}</account_code>
             <username>#{data.username}</username>
             <email>#{data.email}</email>
  ...
  getAccount accountCode, (err, res) ->
    if err
      recurlyPost 'accounts', xmlWithBilling, (err, res) ->
        if err
          callback err, {}
        else
          callback no, res
      ...
      if res.username
        recurlyPut "accounts/#{accountCode}", xml, (err, res) ->
          if err
            ...
            setBilling accountCode, data, (err, res) ->
              if err
                callback err, {}
              else
                callback no, res
...
module.exports = 
  ...
  terminateUserSubscription   : terminateUserSubscription
  getAccount                  : getAccount
  setBilling                  : setBilling
  getBilling                  : getBilling
  setAccount                  : setAccount
  setAccountWithBilling       : setAccountWithBilling
  ...
  addSubscriptionAndAccount   : addSubscriptionAndAccount
