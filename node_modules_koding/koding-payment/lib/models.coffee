scrub = (obj) ->
  for own _, val of obj
    scrub val  if val? and 'object' is typeof val
...
class exports.Adjustment
  ...
  constructor: (xmlJs) ->
    ...
    @uuid         = xmlJs.uuid[0]
    @amount       = xmlJs.total_in_cents[0]._
...
class exports.PaymentMethod
  ...
  constructor: (xmlJs) ->
    ...
    @cardMonth      = xmlJs.month[0]._
    @cardYear       = xmlJs.year[0]._
    @cardCV         = '***'
...
class exports.Plan
  ...
  constructor: (xmlJs) ->
    ...
    @title        = xmlJs.name[0]
    @code         = xmlJs.plan_code[0]
    @desc         = ifString xmlJs.description[0]
    @feeMonthly   = parseInt xmlJs.unit_amount_in_cents[0]['USD'][0]._, 10
    @feeInitial   = parseInt xmlJs.setup_fee_in_cents[0]['USD'][0]._, 10
    @feeInterval  = parseInt xmlJs.plan_interval_length[0]._, 10
...
class exports.Subscription
  ...
  constructor: (xmlJs) ->
    ...
    @plan     = xmlJs.plan[0].plan_code[0]
    @quantity = xmlJs.quantity[0]._
    @uuid     = xmlJs.uuid[0]
    @currency = xmlJs.currency[0]
    @status   = xmlJs.state[0]
    @datetime = xmlJs.activated_at[0]._
    @expires  = xmlJs.expires_at[0]._
    @renew    = xmlJs.current_period_ends_at[0]._
    @amount   = xmlJs.unit_amount_in_cents[0]._

    if xmlJs.pending_subscription
      @pending =
        plan      : xmlJs.pending_subscription[0].plan[0].plan_code[0]
        quantity  : xmlJs.pending_subscription[0].quantity[0]._
...
class exports.Transaction
  ...
  constructor: (xmlJs) ->
    ...
    @amount     = parseInt xmlJs.amount_in_cents[0]._, 10
    @tax        = parseInt xmlJs.tax_in_cents[0]._, 10
    @currency   = xmlJs.currency[0]
    @datetime   = xmlJs.created_at[0]._
    @refundable = xmlJs.refundable[0]._
