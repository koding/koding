...
module.exports = Broker = (function(_super) {
    ...
    return channel.on('setSecretName', function(secretName) {
      ...
      if (!isReadOnly) {
        channel.on('publish', function() {
          ...
          return privateChannel.publish.apply(privateChannel, rest);
        });
      }
  ...
  Broker.prototype.swapPrivateSourceChannel = function(channel) {
    var oldPrivateChannel, privateChannel,
      _this = this;

    privateChannel = channel.privateChannel, oldPrivateChannel = channel.oldPrivateChannel;
    if (oldPrivateChannel != null) {
      return setTimeout(function() {
        oldPrivateChannel.close().off();
        delete channel.oldPrivateChannel;
        return privateChannel.pipe(channel);
  ...
  Broker.prototype.createChannel = function(name, options) {
    var channel, handler, isExclusive, isP2P, isPrivate, isReadOnly, isSecret, routingKeyPrefix, suffix,
      _this = this;

    if (this.channels[name] != null) {
      return this.channels[name];
    }
    isReadOnly = options.isReadOnly, isSecret = options.isSecret, isExclusive = options.isExclusive, isPrivate = options.isPrivate, isP2P = options.isP2P, suffix = options.suffix;
    if (suffix == null) {
    ...
    channel.on('newListener', function(event, listener) {
      ...
      if (channel.privateChannel != null) {
    ...
    if (!(isPrivate || isReadOnly)) {
      channel.on('publish', function(options, payload) {
        var exchange, _ref1, _ref2, _ref3;

        if (payload == null) {
          _ref1 = [options, payload], payload = _ref1[0], options = _ref1[1];
        }
        exchange = (_ref2 = options != null ? options.exchange : void 0) != null ? _ref2 : channel.isP2P ? 'broker' : (_ref3 = channel.exchange) != null ? _ref3 : channel.name;
