...
require.define("/node_modules_koding/koding-broker-client/lib/broker-client/index.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  ...
  module.exports = Broker = (function(_super) {
    ...
    Broker.prototype.handleMessageEvent = function(event) {
      ...
      if (message.event === 'subscribed') {
        return this.emit('subscribed', message.routingKeyPrefix);
      }
    };
    ...
    Broker.prototype.send = function(data) {
      if (this.ws.readyState === SockJS.CONNECTING) {
        throw new Error("INVALID_STATE_ERR");
      }
      if (this.ws.readyState === SockJS.OPEN) {
        this.ws._transport.doSend(JSON.stringify(data));
      }
      return true;
    };

    Broker.prototype.publish = function(exchange, routingKey, payload) {
    ...
    Broker.prototype.subscribe = function(exchange, routingKeyPrefix, callback) {
      var handler,
        _this = this;
      this.send({
        action: 'subscribe',
      ...
      handler = function(prefix) {
        if (prefix === routingKeyPrefix) {
          _this.off('subscribed', handler);
        }
        return callback();
      };
      return this.on('subscribed', handler);
    };

    Broker.prototype.unsubscribe = function(exchange, routingKeyPrefix) {
      return this.send({
        action: 'unsubscribe',
        exchange: exchange,
        routingKeyPrefix: routingKeyPrefix
