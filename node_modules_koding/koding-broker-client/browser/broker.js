...
module.exports = Broker = (function(_super) {
  ...
  Broker.prototype.emitToChannel = function() {
    var channel, event, oldChannelEvent, rest;

    channel = arguments[0], event = arguments[1], rest = 3 <= arguments.length ? __slice.call(arguments, 2) : [];
    if (channel.event != null) {
      oldChannelEvent = channel.event;
    }
    channel.event = this.event;
      ...
      if (event !== 'broker.subscribed') {
        ...
        if (needsToBeRegistered) {
          return _this.on(namespacedEvent, function() {
            ...
            rest = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            return _this.emitToChannel.apply(_this, [channel, event].concat(__slice.call(rest)));
          });
        }
      }
    });
