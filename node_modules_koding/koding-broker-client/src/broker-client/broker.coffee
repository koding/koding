...
module.exports = class Broker extends EventEmitter
  ...
  setP2PKeys:(channelName, { routingKey, bindingKey }, serviceType)->
    ...
    channel.off('publish').on 'publish', producerChannel.bound 'publish'
  ...
  onopen:->
    if @readyState is CLOSED
      ...
      @subscribe args...  for args in oldSubscriptions
  ...
  wrapPrivateChannel:(channel)->
    ...
    channel.on 'setSecretNames', (secretName)=>
      ...
      channel.close() # close the channel—no more messages from upstream

      consumerChannel = @subscribe secretName.publishingName, {isReadOnly, isSecret: yes, exchange: channel.exchange}
      consumerChannel.setAuthenticationInfo {serviceType: 'secret'}
  ...
  subscribe:(name, options={}, callback)->
    return @channels[name]  if @channels[name]
    @subscriptions.push arguments  unless @knownSubscriptions[name]?
