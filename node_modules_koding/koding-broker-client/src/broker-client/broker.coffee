...
module.exports = class Broker extends EventEmitter
  ...
  createChannel:(name, isReadOnly)->
    ...
    channel.once 'broker.subscribed', => @authenticate channel
    ...
    channel.on 'newListener', (event)=>
      if event is 'broker.subscribed'
        ...
        @on event, handler = (routingKeyPrefix1)=>
          if routingKeyPrefix1 is channel.routingKeyPrefix
            @off event, handler
            channel.emit event, routingKeyPrefix1
      else
