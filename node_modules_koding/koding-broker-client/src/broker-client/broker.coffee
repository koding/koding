...
module.exports = class Broker extends `KDEventEmitter.Wildcard`
  ...
  constructor:(ws, options)->
    ...
    { @autoReconnect, @authExchange
      @overlapDuration, @servicesEndpoint } = options
  ...
  persisOldSocketId:->
    ...
    if clientId and oldSocket
      $.cookie "oldSocket", "#{clientId}@#{oldSocket}"
  ...
  connectDirectly:->
    ...
    @ws.addEventListener 'message', => @emit 'messageArrived'
  ...
  sendListenBrokerResubscriptions:(socketId, failCallback)->
  ...
  tryToConnectWithOldSocketId: (failCallback)->
    if oldSocket = $.cookie "oldSocket"
      [clientId, socketId] = oldSocket.split("@")
      if clientId = $.cookie("clientId") is clientId
        @sendListenBrokerResubscriptions socketId, =>
          failCallback?()
      else
        # if clientId has changed clear the oldsocketId
        $.cookie oldSocketId, {erase:yes}
        failCallback?()
    else
      failCallback?()
  ...
  resubscribeBySocketId: ->
    @sendListenBrokerResubscriptions @socketId, =>
      @tryToConnectWithOldSocketId @resubscribeBySubscriptions()
  ...
  resubscribe: (callback) ->
    if oldSocket = $.cookie "oldSocket"
      [clientId, oldSocketId] = oldSocket.split("@")

    socketId = @socketId or oldSocketId
    if @socketId? or oldSocketId?
  ...
  enqueueUnsubscription: (routingKeyPrefix) ->
    ...
    if i is -1
      ...
      @sendUnsubscriptions()  if len is @unsubscriptionThreshold
