...
module.exports = class Broker extends EventEmitter
  ...
  createChannel:(name, options)->
    ...
    { isReadOnly, isSecret, isExclusive, isPrivate, isP2P, suffix } = options
    ...
    channel = new Channel name, routingKeyPrefix, {
      isReadOnly, isSecret, isP2P, isExclusive: isExclusive ? isPrivate
    }
    ...
    unless isPrivate or isReadOnly
      channel.on 'publish', (options, payload)=>
        ...
        exchange = options?.exchange ?
          if channel.isP2P then 'broker' else channel.exchange ? channel.name
  ...
  subscribe:(name, options={}, callback)->
    ...
    channel = @channels[name] = @createChannel name, {
      isReadOnly, isSecret, isExclusive, isPrivate, isP2P, suffix
    }
    @send {
      action: 'subscribe'
