...
module.exports = class Broker extends EventEmitter
  ...
  constructor:(ws, options)->
    @sockURL = ws
    {@autoReconnect, @authChannelName} = options
  ...
  createChannel:(name, {isReadOnly, isSecret})->
    return @channels[name]  if @channels[name]?
    routingKeyPrefix = @createRoutingKeyPrefix name, isReadOnly
    channel = new Channel name, routingKeyPrefix, {isReadOnly, isSecret}
    ...
    channel.on 'setSecretName', (secretName)=>
      ...
      secretChannel = @subscribe secretName, {isReadOnly, isSecret: yes}
      secretChannel.setAuthenticationInfo {serviceType: 'secret'}
      secretChannel.on 'message', (rest...)-> channel.emit 'message', rest...
    ...
    if callback?
      @on 'broker.subscribed', handler = (prefix)=>
        if prefix is routingKeyPrefix
          @off 'broker.subscribed', handler
          callback prefix
    return @channels[name] = @createChannel name, {isReadOnly, isSecret}
