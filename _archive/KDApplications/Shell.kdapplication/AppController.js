...
TerminalClientFactory = (function() {
  ...
  TerminalClientFactory.registerClient = function(name, client) {
    ...
    factory.clients[name] = client;
        if ((_ref = factory.defaultTerminal) != null) {
      _ref;
    } else {
      factory.defaultTerminal = name;
    };
    factory.totalClients++;
        if ((_ref2 = factory.defaultTerminal) != null) {
      _ref2;
    } else {
      factory.defaultTerminal = name;
    };
  ...
  TerminalClientFactory.setDefaultTerminal = function(name) {
    ...
    factory = this.getInstance();
    if (factory.clients[name] != null) {
      return factory.defaultTerminal = name;
    }
  };
...
TerminalClient = (function() {
  ...
  TerminalClient.prototype.setError = function(msg) {
    if (msg != null) {
      return this.view.updatePartial(msg, ".cursor");
    }
  };
  ...
  TerminalClient.prototype.keyPress = function(ev) {
    var k, kc;
    if (ev.which === 118 && (ev.metaKey || ev.ctrlKey)) {
      return true;
    }
  ...
  TerminalClient.prototype.keyDown = function(ev) {
    ...
    k = '';
    kc = ev.keyCode;
    if (ev.which === 86 && (ev.metaKey || ev.ctrlKey)) {
      return true;
    }
    if (kc === 33) {
      k = this.esc_seq("5~");
...
AppController = (function() {
  ...
  AppController.prototype.initiateTerminal = function(callback) {
    console.log("initating terminal");
    var options, view;
    var _this = this;
    view = this.getView();
    options = this.generateTerminalOptions();
    ...
    return this.account.tellKite({
      ...
      toDo: "create",
      withArgs: options
    }, function(error, terminal) {
      // console.log("initated terminal",terminal);
      if (error) {
        err = "Failed to start terminal, please close the tab and try again."
        console.log(err);
        _this.setNotification(err);
        return console.log(error);
      } else {

        _this.terminal = terminal;
        _this.welcomeUser(terminal.isNew);
