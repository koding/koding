(function(){var TerminalErrno, TerminalError;
...
TerminalError = (function() {
  ...
  function TerminalError(error, msg) {
    this.msg = msg != null ? msg : '';
    this.errno = this.lookupError(error);
  }

  TerminalError.prototype.lookupError = function(error) {
    var err, errno, name;
    if ((error.code != null) && (error.name != null)) {
      errno = error;
    } else {
      if (isNaN(error)) {
        errno = TerminalErrno[error];
      } else {
        for (name in TerminalErrno) {
          err = TerminalErrno[name];
          if (err.code === error) {
            errno = err;
            break;
          }
  ...
  TerminalError.prototype.getErrorName = function() {
    return this.errno.name;
  };

  TerminalError.prototype.getErrorCode = function() {
    return this.errno.code;
  };

  TerminalError.prototype.getErrorMessage = function() {
    return this.msg;
  };

  TerminalError.prototype.isEqual = function(error) {
    var errno;
    errno = this.lookupError(error);
    return errno.code === this.errno.code;
  };
...
/*
Singleton terminal client factory
*/

var TerminalClientFactory;

TerminalClientFactory = (function() {

  function TerminalClientFactory() {}

  TerminalClientFactory.getInstance = function() {
    if (this.instance != null) return this.instance;
    this.instance = new this;
    this.instance.clients = {};
    this.instance.totalClients = 0;
    return this.instance;
  };

  TerminalClientFactory.registerClient = function(name, client) {
    var factory, _ref, _ref2;
    factory = this.getInstance();
    factory.clients[name] = client;
    if ((_ref = factory.defaultTerminal) == null) factory.defaultTerminal = name;
    factory.totalClients++;
    if ((_ref2 = factory.defaultTerminal) == null) factory.defaultTerminal = name;
  };

  TerminalClientFactory.createClient = function(options, name) {
    var factory;
    factory = this.getInstance();
    if (name == null) name = factory.defaultTerminal;
    if (!(factory.clients[name] != null)) {
      throw "Client [" + name + "] not registered";
    }
    return factory.clients[name](options);
  };

  TerminalClientFactory.setDefaultTerminal = function(name) {
    var factory;
    factory = this.getInstance();
    if (factory.clients[name] != null) return factory.defaultTerminal = name;
  };

  TerminalClientFactory.isRegistered = function(name) {
    var factory;
    factory = this.getInstance();
    if (factory.clients[name] != null) return true;
    return false;
  };

  TerminalClientFactory.getDefaultTerminal = function() {
    var factory;
    factory = this.getInstance();
    return factory.defaultTerminal;
  };

  TerminalClientFactory.getTotalRegisteredClientsCount = function() {
    var factory;
    factory = this.getInstance();
    return factory.totalClients;
  };

  TerminalClientFactory.forEach = function(callback) {
    var factory, name, _results;
    factory = this.getInstance();
    _results = [];
    for (name in factory.clients) {
      _results.push(callback(name));
    }
    return _results;
  };

  return TerminalClientFactory;

})();

TerminalClientFactory.registerClient('anyterm.js',function(options){
var DiffScript;
DiffScript = (function() {
  function DiffScript() {
    this.source = "";
...
TerminalClient = (function() {
  ...
  function TerminalClient(options) {
    this.view = options.view;
    this.handler = options.handler;
    this.view.getDomElement().css("overflow", "hidden").css("border", "1 solid red");
    this.diffScriptParser = new DiffScript;
    this.kb_buf = '';
  }

  TerminalClient.prototype.setHandler = function(handler) {
    this.handler = handler;
  };

  TerminalClient.prototype.reset = function(size) {};

  TerminalClient.prototype.setError = function(msg) {
    if (msg != null) return this.view.updatePartial(msg, ".cursor");
  };

  TerminalClient.prototype.resize = function(size) {};

  TerminalClient.prototype.write = function(data) {
    var screen;
    screen = this.diffScriptParser.dispatch(data);
    return this.view.updatePartial(screen);
  };

  TerminalClient.prototype.keyPress = function(ev) {
    var k, kc;
    if (ev.which === 118 && (ev.metaKey || ev.ctrlKey)) return true;
    if ((ev.ctrlKey && !ev.altKey) || (ev.which === 0) || (ev.keyCode === 8) || (ev.keyCode === 16)) {
      this.key_ev_stop(ev);
      return false;
    }
    kc = '';
    ...
    if (ev.altKey && !ev.ctrlKey) k = String.fromCharCode(27) + k;
    this.process_key(k);
    this.key_ev_stop(ev);
    return false;
  };

  TerminalClient.prototype.keyDown = function(ev) {
    var k, kc, _ref, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8;
    k = '';
    kc = ev.keyCode;
    if (ev.which === 86 && (ev.metaKey || ev.ctrlKey)) return true;
    if (kc === 33) {
      k = this.esc_seq("5~");
    } else if (kc === 34) {
      k = this.esc_seq("6~");
      ...
      k = this.esc_seq((_ref = ev.shiftKey) != null ? _ref : {
        "25~": "[A"
      });
    } else if (kc === 113) {
      k = this.esc_seq((_ref2 = ev.shiftKey) != null ? _ref2 : {
        "26~": "[B"
      });
    } else if (kc === 114) {
      k = this.esc_seq((_ref3 = ev.shiftKey) != null ? _ref3 : {
        "28~": "[C"
      });
    } else if (kc === 115) {
      k = this.esc_seq((_ref4 = ev.shiftKey) != null ? _ref4 : {
        "29~": "[D"
      });
    } else if (kc === 116) {
      k = this.esc_seq((_ref5 = ev.shiftKey) != null ? _ref5 : {
        "31~": "[E"
      });
    } else if (kc === 117) {
      k = this.esc_seq((_ref6 = ev.shiftKey) != null ? _ref6 : {
        "32~": "17~"
      });
    } else if (kc === 118) {
      k = this.esc_seq((_ref7 = ev.shiftKey) != null ? _ref7 : {
        "33~": "18~"
      });
    } else if (kc === 119) {
      k = this.esc_seq((_ref8 = ev.shiftKey) != null ? _ref8 : {
        "34~": "19~"
      });
    } else if (kc === 120) {
      k = this.esc_seq("20~");
  ...
  TerminalClient.prototype.send = function() {
    var command;
    command = this.kb_buf;
    this.kb_buf = '';
  ...
  TerminalClient.prototype.esc_seq = function(s) {
    return String.fromCharCode(27) + "[" + s;
  };

  TerminalClient.prototype.process_key = function(k) {
    this.kb_buf += k;
    return this.maybe_send();
  };

  TerminalClient.prototype.maybe_send = function() {
    if (this.kb_buf !== '') return this.send();
  };

  TerminalClient.prototype.key_ev_stop = function(ev) {
  ...
  TerminalClient.prototype.key_ev_supress = function(ev) {
    ev.cancelBubble = true;
    if (ev.stopPropagation) return ev.stopPropagation();
  };
...
TerminalClientFactory.registerClient('tty.js',function(options){
/**
...
TerminalClient = (function() {
  ...
  __extends(TerminalClient, this.Terminal);

  function TerminalClient(options) {
    var div, i, _ref;
    this.view = options.view;
    this.handler = options.handler;
    TerminalClient.__super__.constructor.call(this, options.size.cols, options.size.rows, function() {});
    this.element = this.view.getDomElement().get(0);
    this.children = [];
  ...
  TerminalClient.prototype.reset = function() {
    TerminalClient.__super__.reset.apply(this, arguments);
  ...
  TerminalClient.prototype.keyPress = function(ev) {
    return this.keyPressHandler(ev);
  };

  TerminalClient.prototype.keyDown = function(ev) {
    return this.keyDownHandler(ev);
  };

  TerminalClient.prototype.setError = function(msg) {
    this.cursorHidden = true;
    if (msg != null) this.write("[" + msg + "]");
    return this.handler = function() {};
  };

  TerminalClient.prototype.resize = function(size) {
    return TerminalClient.__super__.resize.call(this, size.cols, size.rows);
  };
...
TerminalClientFactory.setDefaultTerminal('anyterm.js');var AppController, KDButtonViewWithMenu, KDCustomHTMLView, KDHitEnterInputView, KDInputView, KDNotificationView, KDTabPaneView, KDTabView, KDView, KDViewController, ShellInputView, ShellView, Shell_UIControls, TabHandleView, framework;
...
KDCustomHTMLView = framework.KDCustomHTMLView, KDViewController = framework.KDViewController, KDView = framework.KDView, KDInputView = framework.KDInputView, KDTabView = framework.KDTabView, KDHitEnterInputView = framework.KDHitEnterInputView, KDTabPaneView = framework.KDTabPaneView, KDButtonViewWithMenu = framework.KDButtonViewWithMenu, KDNotificationView = framework.KDNotificationView;
TabHandleView = (function() {
  __extends(TabHandleView, KDView);
  function TabHandleView() {
    TabHandleView.__super__.constructor.apply(this, arguments);
  }
  TabHandleView.prototype.setDomElement = function() {
    return this.domElement = $("<b>Terminal</b>      <span class='kdcustomhtml terminal icon'></span>");
  };
  return TabHandleView;
})();
AppController = (function() {
  __extends(AppController, KDViewController);
  AppController.prototype.nextScreenDiff = function(data, messageNum) {
    var diff, doThese, i, item, _i, _lastMessageProcessed, _len, _orderedMessages, _ref;
    _lastMessageProcessed = this._lastMessageProcessed, _orderedMessages = this._orderedMessages;
    _orderedMessages[messageNum] = data;
    if (messageNum === _lastMessageProcessed) {
      doThese = [];
      i = _lastMessageProcessed;
      _ref = ((function() {
        var _results;
        _results = [];
        while ((item = _orderedMessages[i++]) != null) {
          _results.push(item);
        }
        return _results;
      })());
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        diff = _ref[_i];
        this.getView().updateScreen(diff);
      }
      return this._lastMessageProcessed = i - 1;
    }
  };
  AppController.prototype.resetMessageCounter = function() {
    console.log('message counter is reset.');
    this._lastMessageProcessed = 0;
    return this._orderedMessages = [];
  };
  AppController.prototype.generateTerminalOptions = function() {
    var options, view;
    view = this.getView();
    options = view.getSize();
    options.callbacks = {
      data: __bind(function(data, messageNum) {
        return this.nextScreenDiff(data, messageNum);
      }, this),
      error: __bind(function(error) {
        var msg;
        this.getView().disableInput();
        msg = "connection closed";
        if (error.msg) {
          msg += "," + error.msg;
        }
        return this.setNotification(msg);
      }, this),
      ping: __bind(function() {
        try {
          return this.terminal.ping();
        } catch (e) {
          return console.log("terminal ping error: " + e);
        }
      }, this)
    };
    options.callbacks.newSession = __bind(function(totalViews) {
      var notification;
  ...
  function AppController() {
    ...
    this.setView(shellView = new ShellView);
    this.resetMessageCounter();
    shellView.registerListener({
    ...
    resetRegexp = /reset\:.*?/;
    shellView.registerListener({
      KDEventTypes: 'AdvancedSettingsFunction',
      listener: this,
      callback: __bind(function(pubInst, _arg) {
        var clientType, functionName;
        functionName = _arg.functionName;
        switch (functionName) {
          case 'clear':
            return this.send("clear\n");
          case 'closeOtherSessions':
            try {
              return this.terminal.closeOtherSessions();
            } catch (e) {
              return console.log("terminal:closeOtherSessions error : " + e);
            }
            break;
          default:
            if (resetRegexp.test(functionName)) {
              ...
              return this.resetTerminalSession(clientType);
            }
        }
      }, this)
    });
  }
  AppController.prototype.setNotification = function(msg) {
  ...
  AppController.prototype.resetTerminalSession = function(type) {
    var options, view;
    this.setNotification("restarting terminal");
    view = this.getView();
    view.reset(type);
    try {
      this.terminal.kill();
    ...
    options = this.generateTerminalOptions();
    ...
    return this.account.tellKite({
      ...
      if (error) {
        return this.setNotification("Failed to start terminal : " + error);
      } else {
        this.setNotification();
        this.terminal = terminal;
        return this.welcomeUser(true);
      }
    }, this));
  };
  ...
  AppController.prototype.bringToFront = function() {
    this.propagateEvent({
    ...
    appManager.addOpenTab(this.getView(), 'Shell.kdapplication');
    return this.getView().input.inputSetFocus();
  };
  ...
  AppController.prototype.closeView = function(view) {
    appManager.removeOpenTab(this.getView());
  ...
  AppController.prototype.getKiteIds = function(options, callback) {
    return this.account.fetchKiteIds({
      kiteName: "terminaljs"
    }, function(err, kiteIds) {
      console.log("fetch", arguments);
      if (!err) {
        this.kiteIds = kiteIds;
        return typeof callback === "function" ? callback(null, kiteIds) : void 0;
      } else {
        return typeof callback === "function" ? callback(err) : void 0;
      }
    });
  };
  AppController.prototype.pickAResponsiveKite = function(options, callback) {
    var account, done;
    done = false;
    account = this.account;
    return this.getKiteIds({
      kiteName: "terminaljs"
    }, function(err, kiteIds) {
      var kiteId, _i, _len, _results;
      console.log("kids", kiteIds);
      if (!err) {
        _results = [];
        for (_i = 0, _len = kiteIds.length; _i < _len; _i++) {
          kiteId = kiteIds[_i];
          _results.push(__bind(function(kiteId) {
            console.log("asking", kiteId, "if available");
  ...
  AppController.prototype.initiateTerminal = function(callback) {
    var options, view;
    view = this.getView();
    options = this.generateTerminalOptions();
    ...
    return this.account.tellKite({
      ...
      if (error) {
        this.setNotification("Failed to start terminal, please close the tab and try again.");
        return console.log(error);
      } else {
        this.terminal = terminal;
        this.welcomeUser(terminal.isNew);
        return typeof callback === "function" ? callback(terminal.totalSessions) : void 0;
      }
    }, this));
  };
  AppController.prototype.loadView = function(mainView) {
    return this.initiateTerminal(__bind(function(totalSessions) {
      mainView.registerListener({
        KDEventTypes: "resize",
        listener: this,
        callback: this.resizeTerminal
      });
      return mainView.input.on("data", __bind(function(cmd) {
        return this.send(cmd);
      }, this));
    }, this));
  };
  AppController.prototype.welcomeUser = function(isTerminalNew) {
    var username, welcomeText;
    if (isTerminalNew) {
      ...
      return this.send("" + welcomeText + "\n");
    }
  };
  AppController.prototype.resizeTerminal = function() {
    var options;
    options = this.getView().getSize();
    try {
      return this.terminal.resize(options.rows, options.cols);
    } catch (e) {
      return console.log("terminal::resize error " + e);
    }
  };
  AppController.prototype.send = function(command) {
    try {
      return this.terminal.write(command);
    } catch (e) {
      return console.log("terminal::write error : " + e);
    }
  };
...
ShellView = (function() {
  __extends(ShellView, KDView);
  function ShellView() {
    ShellView.__super__.constructor.apply(this, arguments);
    this.setClass("terminal-tab");
    this.$().css("overflow", "auto");
    this.listenWindowResize();
    this.clientType = TerminalClientFactory.getDefaultTerminal();
  }
  ShellView.prototype.viewAppended = function() {
    this.addSubView(this.screen = new KDCustomHTMLView({
      tagName: 'pre',
      cssClass: "terminal-screen"
    }));
    this.addSubView(this.input = new ShellInputView({
      bind: "paste",
      position: {
        top: -4000,
        left: -4000
    ...
    this.addSubView(this.controllMenu = new Shell_UIControls({
      cssClass: "shell-advanced-settings-wrapper",
      delegate: this
    }));
    this.initiateClient();
    return ShellView.__super__.viewAppended.apply(this, arguments);
  };
  ShellView.prototype.destroy = function() {
    this.removeSubView(this.screen);
    this.removeSubView(this.input);
    this.removeSubView(this.controllMenu);
    if (this.client) {
      this.client.setError("");
      return delete this.client;
    }
  };
  ShellView.prototype.disableInput = function() {
    return this.input.stop();
  };
  ShellView.prototype.initiateClient = function(type) {
    var clientOptions;
    if (!(type != null)) {
    ...
    this.client = TerminalClientFactory.createClient(clientOptions, type);
    this.input.setHandler(this.client);
    return this.input.inputSetFocus();
  };
  ShellView.prototype.reset = function(type) {
    return this.initiateClient(type);
  };
  ShellView.prototype.click = function() {
    return this.input.inputSetFocus();
  };
  ShellView.prototype.updateScreen = function(data) {
    var _ref;
    return (_ref = this.client) != null ? _ref.write(data) : void 0;
  };
  ShellView.prototype._windowDidResize = function() {
    var _ref;
    if ((_ref = this.client) != null) {
      _ref.resize(this.calculateSize());
    }
    return this.propagateEvent({
      KDEventType: "resize"
    });
  };
  ShellView.prototype.calculateSize = function() {
    var obj;
    obj = {
      cols: Math.floor(this.screen.getWidth() / 8),
      rows: Math.floor(this.screen.getHeight() / 16)
    };
  ...
  ShellView.prototype.getSize = function() {
    return this.calculateSize();
  };
  return ShellView;
})();
Shell_UIControls = (function() {
  __extends(Shell_UIControls, KDView);
  function Shell_UIControls(options) {
    var id;
    Shell_UIControls.__super__.constructor.call(this, options);
    this.menu = {
      type: "contextmenu",
      items: [
        {
    ...
    if (TerminalClientFactory.getTotalRegisteredClientsCount() > 1) {
      TerminalClientFactory.forEach(__bind(function(clientName) {
        this.menu.items.push({
          title: "Restart with " + clientName,
          id: id,
          parentId: null,
          "function": "reset:" + clientName
        });
        return id++;
      }, this));
    }
  }
  Shell_UIControls.prototype.viewAppended = function() {
    var settingsButton;
    settingsButton = new KDButtonViewWithMenu({
      ...
      callback: function(event) {
        return settingsButton.contextMenu(event);
      }
    });
    settingsButton.registerListener({
      ...
      callback: __bind(function() {
        return this.getDelegate().propagateEvent({
          KDEventType: "AdvancedSettingsFunction"
        }, arguments[1]);
      }, this)
    });
    return this.addSubView(settingsButton);
  };
  return Shell_UIControls;
})();
ShellInputView = (function() {
  __extends(ShellInputView, KDInputView);
  function ShellInputView(options) {
    ShellInputView.__super__.constructor.call(this, options);
    window.tt = this;
  }
  ShellInputView.prototype.stop = function() {
    return this.client = null;
  };
  ShellInputView.prototype.setHandler = function(client) {
    this.client = client;
    return this.client.setHandler(__bind(function(data) {
      return this.emit("data", data);
    }, this));
  };
  ShellInputView.prototype.focus = function() {
    return this.parent.setClass("terminal-on-focus");
  };
  ShellInputView.prototype.blur = function() {
    return this.parent.unsetClass("terminal-on-focus");
  };
  ShellInputView.prototype.keyPress = function(event) {
    var _ref;
    return (_ref = this.client) != null ? _ref.keyPress(event) : void 0;
  };
  ShellInputView.prototype.keyDown = function(event) {
    var _ref;
    return (_ref = this.client) != null ? _ref.keyDown(event) : void 0;
  };
  ShellInputView.prototype.paste = function(event) {
    return setTimeout(__bind(function() {
      var pastedContent;
      pastedContent = this.inputGetValue();
      this.emit('data', pastedContent);
