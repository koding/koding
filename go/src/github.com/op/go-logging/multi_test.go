...
func TestMultiLogger(t *testing.T) {
	...
	if "log" != MemoryRecordN(log1, 0).Formatted() {
		t.Errorf("log1: %v", MemoryRecordN(log1, 0).Formatted())
	}
	if "log" != MemoryRecordN(log2, 0).Formatted() {
		t.Errorf("log2: %v", MemoryRecordN(log2, 0).Formatted())
	}
}
