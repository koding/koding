...
// ListLanguagesFoRepo returns list of languages in the repository.
//
// GitHub API docs: http://developer.github.com/v3/repos/#list-languages
func (s *RepositoriesService) ListLanguagesFoRepo(owner, repo string) (*RepoLanguages, error) {
	url_ := fmt.Sprintf("repos/%v/%v/languages", owner, repo)
	req, err := s.client.NewRequest("GET", url_, nil)
	if err != nil {
		return nil, err
	}

	listOfLangs := new(RepoLanguages)
	_, err = s.client.Do(req, listOfLangs)
	return listOfLangs, err
}
