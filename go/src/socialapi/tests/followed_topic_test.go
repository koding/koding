		...
		Convey("While testing followed topics", t, func() {
			Convey("First Create User", func() {
				...
				Convey("user should be able to follow one topic", func() {
					channelParticipant, err := rest.AddChannelParticipant(topicChannel1.Id, account.Id, ses.ClientId, account.Id)
				...
				Convey("user should be able to follow two topic", func() {
					...
					channelParticipant, err = rest.AddChannelParticipant(topicChannel2.Id, account.Id, ses.ClientId, account.Id)
					So(err, ShouldBeNil)
					So(channelParticipant, ShouldNotBeNil)
				...
				Convey("user should not be a participant of the un-followed topic", func() {
					...
					channelParticipant, err = rest.DeleteChannelParticipant(topicChannel1.Id, account.Id, ses.ClientId, account.Id)
					So(err, ShouldBeNil)
					So(channelParticipant, ShouldNotBeNil)
