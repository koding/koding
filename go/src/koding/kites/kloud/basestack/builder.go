...
// Builder implements common functionality for stack providers (aws, vagrant).
//
...
// in *Builder, *BaseStack, *BaseMachine, *stackplan.Builder
...
func (b *Builder) New(p *Provider) *Builder {
	bCopy := *b
	bCopy.Provider = p
...
func (b *Builder) Machine(ctx context.Context, id string) (Machine, error) {
...
func (b *Builder) BaseMachine(ctx context.Context, id string) (*BaseMachine, error) {
...
func (b *Builder) Stack(ctx context.Context) (Stack, error) {
...
func (b *Builder) BaseStack(ctx context.Context) (*BaseStack, error) {
	bs := &BaseStack{
		Planner: &stackplan.Planner{
			Provider:     b.Provider.Name,
			ResourceType: b.Provider.resourceName(),
		},
		KlientIDs: make(stackplan.KiteMap),
		Klients:   make(map[string]*stackplan.DialState),
	}
	...
	bs.Log = b.Log.New(req.GroupName)
...
func (b *Builder) FetchCredData(bm *BaseMachine) error {
	data := map[string]interface{}{
		bm.Machine.Credential: object.Inline(bm.Credential, bm.Bootstrap),
	}

	return b.CredStore.Fetch(bm.Username(), data)
}

func (b *Builder) ValidateUser(user *models.User, users []models.MachineUser, r *kite.Request) error {
	// give access to kloudctl immediately
	if stack.IsKloudctlAuth(r, b.KloudSecretKey) {
		return nil
	}
