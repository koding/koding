...
func (b *Builder) Lock(id string) error {
	err := b.DB.Run("jMachines", func(c *mgo.Collection) error {
	...
	if err != nil {
		b.Log.Error("Storage get error: %s", err)
...
func (b *Builder) Unlock(id string) {
	b.DB.Run("jMachines", func(c *mgo.Collection) error {
		return c.UpdateId(
			bson.ObjectIdHex(id),
