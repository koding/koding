...
func (cmd *TeamInit) Run(ctx context.Context) error {
	...
	cmds := map[string]string{
		"auth":      fmt.Sprintf("kloudctl team auth -p %s -team %s -creds %s", cmd.Provider, cmd.Team, creds),
		"bootstrap": fmt.Sprintf("kloudctl team bootstrap -p %s -team %s -creds %s", cmd.Provider, cmd.Team, creds),
		"plan":      fmt.Sprintf("kloudctl team plan -p %s -team %s -tid %s", cmd.Provider, cmd.Team, user.StackTemplateID),
		"apply":     fmt.Sprintf("kloudctl team apply -p %s -team %s -sid %s", cmd.Provider, cmd.Team, user.StackID),
	}

	p, err := json.MarshalIndent(cmds, "", "\t")
	if err != nil {
		return err
	}
