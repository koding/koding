...
go/bin/gometalinter.v1 --concurrency=10 --fast --enable-gc --config="$GOPATH/src/.gometalinter.json" $GOPATH/src/socialapi/... $GOPATH/src/koding/... --deadline=60s | grep -v /vendor/
